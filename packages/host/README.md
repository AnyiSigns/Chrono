...
`offline.ts`（`seed` / `verify` / `replay`）、`paths.ts`（落盘路径单点）、`service-link.ts`（服务协议宿主侧）、
...
const host = await startHost({ root }) // 也可注入更短 callTimeoutMs（测试）
// … 客户端连 host.socket 提交 …
await host.stop()
```
...
node packages/boot/main.ts start
