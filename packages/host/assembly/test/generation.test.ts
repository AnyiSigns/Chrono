...
describe('classifyGenerationChange（A6 换代判据）', () => {
  ...
  it('仅 term 成员内容变化 → data（进程热生效）', () => {
    ...
    expect(classifyGenerationChange(v1.world, v1.gen, v2.world, v2.gen)).toBe('data')
  })
  ...
  it('execute 成员内容变化 → code（起新服务）', () => {
    ...
    expect(classifyGenerationChange(v1.world, v1.gen, v2.world, v2.gen)).toBe('code')
  })
  ...
  it('声明不可读（世代 payload 缺 def）→ code（保守起新服务）', () => {
    ...
    expect(classifyGenerationChange(v1.world, broken, v1.world, v1.gen)).toBe('code')
    expect(classifyGenerationChange(v1.world, v1.gen, v1.world, broken)).toBe('code')
  })
  ...
  it('readPluginDeclOfGen 按指定世代读声明（非 active 也能读）', () => {
    ...
    const readOld = readPluginDeclOfGen(v2.world, v1.gen)
