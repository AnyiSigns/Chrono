// 分片落盘 + 按需加载的量化对比（小基准）：
// 同一世界写 v2 分片（索引 + body 分片），对比「索引」与「内联整世界」的体积、读基础世界的耗时、
...
import { H, canonicalJson, worldRev } from '../../kernel/index.ts'
...
function prepare(
  count: number,
  pad: number,
): { root: string; rev: Hash; world: World; v2File: string; inlineBytes: number } {
  ...
  // 内联整世界（v1 形态）的字节数：仅作体积对照
  const inlineBytes = Buffer.byteLength(canonicalJson(world as unknown as Json), 'utf8')
  return { root, rev, world, v2File, inlineBytes }
}
...
function log(
  ...
  console.log(
    `[shard-bench:${label}] defs=${count} pad=${pad}B 内联=${(inlineBytes / 1024).toFixed(0)}KiB | ` +
      `v2 索引=${(indexBytes / 1024).toFixed(1)}KiB 读=${v2.ms.toFixed(1)}ms 常驻Δ=${(v2.heap / 1024).toFixed(0)}KiB${extra}`,
  )
}

describe('基础世界分片量化对比（索引 vs 内联整世界）', () => {
  const roots: string[] = []
  afterEach(async () => {
  ...
  it('多小 def（1200×1KiB）：索引远小于内联，body 零常驻、零读盘', () => {
    const { root, rev, world, v2File, inlineBytes } = prepare(1200, 1024)
    roots.push(root)
    const indexBytes = statSync(v2File).size
    ...
    expect(indexBytes).toBeLessThan(inlineBytes / 8)
  ...
  it('大 body（48×64KiB）：解析收益明显，读索引不常驻 body', () => {
    const { root, rev, v2File, inlineBytes } = prepare(48, 64 * 1024)
    roots.push(root)
    const indexBytes = statSync(v2File).size
    ...
    log('large', 48, 64 * 1024, { ms: v2Ms, heap: v2.heap }, inlineBytes, indexBytes, '')
  })
})
