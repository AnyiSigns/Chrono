// 入站面收紧：插件为浏览器另开了 127.0.0.1 HTTP 端口，浏览器里任意页面都能向它发请求。
// 故在此挡住三类越界：DNS rebinding（Host 伪造）、跨站请求（Origin 伪造）、
// 以及不触发 CORS 预检的「简单请求」（写方法非 JSON 体）。
