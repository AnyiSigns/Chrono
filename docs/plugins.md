...
每条都对应 `docs/host.md` §六 的一条不变量，破了就是插件没写对：

1. **不 import 内核**：不算哈希、不校验、不写链；键 / 哈希 / 校验 / 写链全在宿主。
2. **只提交内容与效果请求**：`put` / `batch` 载荷 + `EffRequest`。
3. **不与其他插件直连**：效果一律经宿主（保 `EffectAudit`）。
4. **依赖只走 `pins`，且可相互依赖**：A 的 `pins` 写 B 的能力（名 → B 的 def 哈希）；A 的代码 / term 只写**能力类名 + 方法名**，宿主按 `pins` 路由。**不 import、不共享进程内对象、不直连**；body 里不得出现结构性引用（规矩 A）。漏写 `pins` 会静默失效。
...
- **依赖联动**：B 换代 → A 的 `pins` 哈希对不上 → A `stale()` → 宿主重新装配 A（特性，不是 bug）。
- **`pins` 闭包必须是 DAG**；成环 = 装配错误（拓扑序无解）。
