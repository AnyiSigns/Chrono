...
> ⚠️ **开工前必读 §1.13「做插件前必须先落地的内核 / 宿主改动」**：多数插件依赖内核 / 宿主侧尚未落地的改动（run 级并发、投影闭包解析、受保护 pins、③ 目录、密钥面、线程控制、事件 emitter、资产 / 沙箱面…）。**在旧代码上开做插件会大面积返工**——务必先核对 §1.13 的前置清单。测试用.env里的模型配置
>
...
6. **数据身份 vs 能力身份**：pins 只用于**能力端点**；**纯数据身份**（无 execute：input / config / short-memory / vendor-* / skill / agents）按字面身份名读写投影，不参与路由；**有 execute 且声明能力类的身份**（session / memory-store / compress / **loop-policy** …）可作 pins 端点。**有 execute 的身份其 `+` 投影读一律由调用方入口 term 完成、经 args/bag 传入**（服务不读投影，通则见 `docs/plugins.md`）。**配套两条（2026-09-20 收口，防「读取义务登记在服务侧、调用方未承接」）**：
   - **承接义务**：服务的 `+` 列表只是**需求声明**；调用方（入口 term 所在插件）必须在**自己**的依赖行 `+` 登记同样的读取面并在 bag 装配时实际读取——双侧各记一条（与版本提升同纪律）。读取面以 **§1.14「bag 装配总表」**为权威。
   - **策略 / 规则数据住世界数据世代 body**（与第 9 条推论一致）：guard 危险规则、#25 档位映射、#28 忽略表、#34 别名清单、#37 服务器清单等一律住**本身份数据世代 body**（热改 = 数据换代），由调用方入口 term 读出随 bag 传入（如 `bag.guard_rules` / `bag.tier`）；**不住包内 schema**（schema 是身份契约、出生即冻结，`host.md` §五 源码）。
7. **世界是真源**：配置 / 会话 / 厂商全部写成世界数据（可回滚、可回放、单写者 CAS）；本地文件只能当导出或缓存。
8. **密钥不进世界**：vendor 只存 `auth_ref`（`kind` = `local` | `env`，`name`）；本体住用户本地文件（`local`）或进程环境（`env`），只由 #24 `secrets` 解析成**解析结果**（明文仅存调用方进程内存——2026-09-20 修订术语，原「短时句柄」）。
...
> 每行的完整设计见各插件目录 `plugins/<身份>/DESIGN.md`。**夹具说明（2026-09-19 澄清，2026-09-20 修订）**：`fixtures/plugins/` 现存放宿主测试用 toy 包（`toy-alpha` / `toy-beta` / `toy-python`，见 `docs/plans/host-plan.md` S4.5）与 `model-stub`（确定性 `model` 夹具，供 #12 / #33 / #14 测试——**当前仅 DESIGN.md，包骨架随 #12 波次补齐**；与 #12 同声明 `model` 能力类，**二者互斥装载**：测试世界用夹具替换 #12，非共存）；**交付插件的逐份夹具 `fixtures/plugins/<身份>/DESIGN.md` 随实现波次补齐**（W0–W7 各波建包时同步建夹具），当前不存在 ≠ 缺失。
...
| 11 | `session` | 会话数据 + 提交服务（原子写计划）：**消息各自成 def + 链式 `head`**、会话元数据进 body；**历史读取 = H1 投影全量 `refs`，`chat.history` 入口 term 按 `before` / `limit` 切片**；**`select` 切 `current`（落账后发 `thread.updated`）** | 搜索 / 朗读 / 调模型；写只限自身 + `input` 清槽（经计划通道）；不删消息、不做去重 | `+` 1（**槽体 / 会话数据由调用方入口 term 读出随 args 传入**，服务不读投影）；`<-` 13（投影）、14、16、27（pins：`subagent.send`→`deliver`）、33（pins） |
| 12 | `model-protocol` | 多 SDK / 三协议 HTTP/SSE（**混合**：三协议自实现 + 怪厂商 SDK）+ **韧性（重试 / 退避 / 限流 / 流断重连）** + `discover`（URL→列表）+ `profile`（models.dev→所选模型档案，**定期后台同步**）+ `vendors`；**`sync` 方法为定时触发入口（宿主直接调方法；所需 `#2` / `#4–10` 投影片段由宿主按 `schema.periodic.reads` 注入 bag，服务不读投影）**；**`model.complete`（非流式单次补全，供 #49）** | 降级链（归 33 服务内判定 + #34）/ 落账 / 密钥解析（归 24）/ 厂商适配数据（归 4–10） | `->` 24（pins：密钥解析）；`+` 2（连接实例与所选模型）、4–10（SDK 与怪癖）——**eff 路径由调用方入口 term（#14 / #17 / #33 bag）读出随 bag 传入，服务不读投影**；`<-` 14、17（pins）、19、22、33、34、49 |
| 13 | `context-window`（TS + Rust tokenizer） | **上下文调配器**：候选汇集（**含本轮用户消息，自 `#1` 槽**）→ 结构化 → 去重（规范化留最新）→ 预算建模 + token 计数（**按 def 哈希缓存 + 前缀和**）→ 配额分配 → 裁剪（保工具对）→ 来源调度 → 冲突消解 → 前缀缓存排序（**稳定前缀 = 系统提示 + 工具 schema，L2 移出**）→ 按方言格式化（含多模态）→ 组装清单（事件，载荷带 `run`/`thread`）→ 75% 触发（**基数 = 可用预算**）→ **交错引导语**；**读时 TTL 过滤**（注入前校验 #3 `expires_at`，过期 L1 不注入）；产出 messages 与请求参数。**不是回合第一个服务**（召回段 #22 先写 bag）；**系统提示 / 工具 schema / 记忆 / 会话 / 配置由调用方入口 term 读后随 bag 传入**（不 pin #27 / #33） | 调模型 / 写世界 / 压缩本体（归 19）/ 模板引擎 | `+` 1、2、3、11（**由 #14 / #33 入口 term 读出随 bag 传入，服务不读投影**）；`<-` 14（pins）、33（pins）；版本提升：22 加记忆引用字段（`bag.recall`）、36 加技能片段（`bag.skills`）、3 注入契约（含读时 TTL 过滤） |
| 14 | `chat` | 命令面（send / history / **resume**，history 支持 `conversation` / `before` / `limit` 窗口）+ 回合管道 term + 接线 + **bag 装配者**（读 `#2` 连接与所选模型、`#1` 槽、`#3` 记忆、`#11` 会话供下游 bag） | 写世界 / 分支策略 / 自建循环（归 33） | `->` 11、12、13、**33（`loop-policy`：入口 term eff `loop-policy.interpret`）**、**49（首条消息标题段）**；`+` 1（读槽判分支）、2、3、11（装配 bag）+ **33 六类条目 / 35 / 36 / 41 / 43 / 47（interpret bag）**（§1.14 总表）；`<-` 16、18（按名）；版本提升：33 替换其管道；**34：静态管道不接降级链（term 无动态端口），router pin 与备选别名 pin 归 #33** |
| 15 | `ui-shell` | 壳：**浏览器唯一入口（主端口 + 入站桥）** / 布局槽 / 路由 / 反代 / token·图标·文案表 / 断线横幅 / **全局 toast** / **跨 slot 视图状态 `api.uiState`** | 渲染业务面板 / 业务判定 / 读投影 | `host`（pins：headless 条目字节经 `host.source.read` 取——2026-09-20 补）；`~` 2（`config.read`，判"无配置"）；`<-` 16、17、18、38、39、40、46（挂载 / headless） |
...
| 17 | `ui-settings` | 引导页（模板 / 自定义同一流程）+ 设置模态 S7–S13（通用 / 模型 / 插件 / 技能 / **记忆** / **编排** / 关于） | 对话视图 / 判定 / 存密钥（只存 `auth_ref`）/ 记忆本体与维护（归 3、19、21–23） | `->` 12（pins：vendors / discover / profile）、**24（pins：`secrets.list`，入口 = 只读命令 `secrets.status`）**；**版本提升**：22/23 到位后新增 `retrieval` / `memory-maintenance` pins（S12，双方各记一条）；`+` 1、2、3、21、33、35、43（投影读 `model.probe` / 记忆 / 图 / 人格 / 台账，入口 term 装配）；`~` 2、`notify.state`（按名，归 #38 声明）；订阅 `api.uiState`（`boot_mode` / `settings_open`）；写 = 直写 2；`<-` 15 |
...
| 19 | `compress` | **压缩引擎**（三种）：`summarize`（会话摘要 → L1）/ `compact`（上下文压缩，75% 触发）/ `extract`（抽 2–3 条 → L2）；`mode` 可纯算法或语义 | 直接写链（只返回计划）/ 检索（归 22）/ 去重合并与固化（归 23）/ 触发判定（阈值归 13、准则归 33） | `->` 12（semantic 模式）、**20（去重向量化，执行件，投影读替代不了调用）**；`+` 3、11（**由调用方入口 term / #33 读出随 bag 传入，服务不读投影**）；`<-` 27（记忆工具）、**23（需要摘要时）** |
...
| 22 | `memory-retrieval`（Rust） | L3 召回：查询构造 / 多查询 → 向量 + 暴力余弦 top-k → 工作区范围 / 标签 / 来源过滤 → 时间衰减 → 与在上下文内容去重 → MMR + 语义重排 → 预算感知截断（**召回预算由 #33 按 `thresholds` 写 `bag.recall_budget`；#13 配额分配为最终截断兜底**） | 写入 / 压缩 / 向量化实现（归 20）/ 存储实现（归 21）/ 组装（归 13） | `->` 20、21、12（多查询 / 重排，可关）；`+` 3、11（**由 #33 从自身 bag 传入，服务不读投影**）；`<-` 33 管道（recall 节点，随 #33 图版本插入）、27（记忆工具）；提升 13（加记忆引用字段 `bag.recall`） |
| 23 | `memory-consolidate` | **记忆维护（全层）**：L1 TTL 清理（24h）+ L2 去重合并 + L2→L3 固化 + L3 过期 / 遗忘 / 删除计划 + agent 顺带清理候选（去重用本地向量余弦）；能力类 `memory-maintenance` | 调模型（交 19）/ 直接写链 / L3 存储实现（归 21）/ 向量计算（归 20） | `->` 19、20、21；`+` 3、11、21（**周期路径经 `schema.periodic.reads` 机械注入、agent 路径由调用方入口 term 读出传入，服务不读投影**）；`<-` 27（记忆工具：`memory.candidates` 等显式工具）；**`consolidate`/`sweep` 周期由宿主定时触发**（周期住 schema）；**不再挂 #33 回合尾**（旧口径作废，2026-09-20 双方确认） |
| 24 | `secrets` | **唯一密钥面**（模型族 + 工具族）：本体住**宿主侧用户本地文件**（世界排除）+ `auth_ref` -> **解析结果**（明文仅存调用方进程内存）；`secrets.put` 走宿主入站面；**`resolve` 结果经宿主审计脱敏**（只落 `{name,kind,has}` 描述） | 明文落账 / 明文进世界 / 明文进审计 / 明文进 config 导出 | `+` 2（`auth_ref` 由 #12/#29 上游入口 term（#14/#33）读出后随 bag 下传，§1.14；服务不读投影）；`<-` 12（模型族）、29（`tool-shell`，经 #25 exec `env` 通道）、**17（S8：只读命令 `secrets.status`）** |
| 25 | `sandbox`（Rust） | 隔离执行（多实现可换）：Rust 原生 OS 级（win32 / linux）+ Docker；一次性 `exec`（**`args.env` 通道**）+ **结构化文件操作 `fsop`（S2 已展开）** + 资源上限；`caps` 驱动 fs **与 net 范围**（四档钳制，越界 `fs_denied` / `net_denied`）+ **消费批准后的一次性 `caps.grant`（由 #33 在裁决放行时构造、随 `tool.dispatch` bag 下传）**；档位映射表住**本身份数据世代 body** | 工具语义 / 审批判定（归 26）/ 危险定义（归 26）/ 长驻交互会话 / 永不发升级（只拒绝） | `+` 2（`tier` 由调用方入口 term 读 `permission` 后随 bag 传入）；`<-` 28–31 |
| 26 | `guard` | 工具调用语义门（纯函数）：**只判「工作区外 / 危险操作 / 外部 MCP / 结构写高危」是否升级弹卡**（4 档 fs 强制归 #25；**危险规则住本身份数据世代 body**，由 #14 入口 term 读出随 `bag.guard_rules` 传入，热改 = 数据换代） | fs 范围强制（归 25）/ 审批 UI / 等待审批 / 调 32 / realpath 权威判定（归 25，词法预判在此） | `+` 2（`tier` 由调用方入口 term 读后随 bag 传入）；`<-` 33（`tool.gate` 主路径，按 call 逐项）、27（兜底，批级一次） |
| 27 | `tools` | 工具注册与派发（**`tool` 端口契约由本插件冻结**，见 `plugins/tools/DESIGN.md`）：**两种提供者**——`describe`/`invoke` 工具类 + **能力类工具绑定**（tool 名 → class+method+argsSchema+render）；工具描述**四要素必填**（行为意图 / 使用时机 / 参数语义 / 使用边界，缺一 `bad_tool_decl`）；**整批 `calls[]` 一次 eff、进程内并发扇出**（并发上限住 schema）；工具卡 **`render` 描述符**随消息快照进 #11；**写类工具的计划值由 #33 `tool.dispatch` 冒泡并入顶层 `$directives`** | 具体工具实现 / 审批流程 | `->` 26（pins：先过语义门）；**工具提供者按各自类名 pin**：`tool-fs`→28、`tool-shell`→29、`tool-http`→30、`tool-browser`→31、`mcp`→37、`plugin-admin`→42、`orchestration-admin`→45、`todo`→47、`question`→48、`session`→11（`subagent.send`/`status`）、**`evolve-metrics`→44（`record`）**；记忆工具（能力类绑定）：`compress`→19、`memory`→21、`retrieval`→22、`memory-maintenance`→23（含 `memory.candidates` 具名工具，见 `plugins/tools/DESIGN.md`）；**`host`→保留身份（pins：`subagent.resume` / `subagent.terminate` 转交 `host.thread.*`）**；`+` 37 / 11 / 41（外部工具清单与执行根由**调用方入口 term 读投影后随 bag 传入**，服务不读投影，D8；**`tier` 由 #33 随 dispatch bag 下传**，服务不自读 `#2`）；`<-` 33 管道（pins；`#14` v1 不 pin 本插件） |
| 28 | `tool-fs`（Rust） | 文件工具：`read` / `edit` / `glob` / `grep`（**工作区内 + 区外**结构化读写；区内 / 区外由 #25 档位强制、区外经 #26 升级） | 绕过 guard / sandbox / 通用命令执行（归 29） | `->` 25、`host`（pins：二进制读写经 `host.asset.put/get`——2026-09-20 补） |
| 29 | `tool-shell` | 命令执行工具 | 绕过 guard / sandbox | `->` 24、25 |
| 30 | `tool-http` | 联网检索与抓取：`websearch`（**多免费源 · 零配置 · 无 API key**）/ `webfetch`（无状态） | 绕过 guard / sandbox / 有会话与 JS 渲染（归 31）/ 需 key 的搜索源 | `->` 25、`host`（pins：二进制响应经 `host.asset.put`——2026-09-20 补） |
| 31 | `tool-browser` | 浏览器自动化：`webbrowser`（会话 / 渲染 / 交互 / 截图） | 绕过 guard / sandbox / 无状态抓取（归 30） | `->` 25（声明级 `caps.net` 钳制）、`host`（pins：截图 / 下载经 `host.asset.put`——2026-09-20 补）；浏览器进程自管 spawn（不经 #25 exec，保留例外） |
| 32 | `approval` | 审批流程与回执（无命令面；**队列进世界**、多项、整批裁决；**超时挂起 + 可配时长、不自动拒绝**；**跨 run 挂起/续跑**、裁决清槽；args 只存摘要 / 资产引用）；**item 带 `thread` 与 `resume:{command:'chat.resume', args}` 游标——裁决命令（#39 声明）的入口 term 产 `[eval(command:'chat.resume', args 含裁决), write(记裁决+清槽)]` 续跑计划（H5 v1 口径，经 plan 通道按命令名解析）** | 渲染（卡片归 39）/ 工具派发 | `+` 1；`<-` 39（pins：命令入口）；事件与 39 约定 |
| 33 | `loop-policy` | **唯一的策略 / 图解释器**（**服务自驱**：图执行住 execute、`pre`/`post`/`when` 为服务内声明式规则、节点经反向调用 `port.call` 派发、**每步记入回合尾 `trace.eff_log`**）：回合管道 + 图执行 + 审批往返（**跨 run 挂起/续跑**，经 `chat.resume` 恢复）+ **回合尾写 trace / 证据 / 队列项与提案扫描**（不做记忆维护——归 #23 宿主周期）；图/策略住本身份的数据世代；六类条目（契约 / Scope / **提示词** / 图 / 阈值 / 拒绝码）；**系统提示词住 `prompts.system`**（行为准则 + 产品事实、只谈意图、**禁工具标识符**），由 `context.assemble` 阶段写进 bag 交 #13 | 自建特权引擎（宿主不给特殊路径）/ 节点实现 / 直接写链 / 多图选图 / 记忆维护触发（归宿主周期） | `->` 11、12、13、22、26、27、32、**34（`agent.step` 失败的降级判定）**、**44 `evolve-metrics`**（pins 枚举即节点类型空间，见其 DESIGN「pins」）；`+` 35、36、41、43、47 + **33 六类条目与 `bag.tier` / `bag.guard_rules` / `bag.workspace_root`**（**均由 #14 入口 term 读投影装配**，服务不读投影，§1.14 总表）；`<-` 14（版本提升：替换其管道，入口 term eff `loop-policy.interpret`；**title 旁路段保留归 #14**）、17（S13 投影读） |
| 34 | `router` | 模型 / 能力选择（**选别名端口名**；别名 pin 各指向一实现，目标须显式声明该别名能力类）；**调用方 = #33 服务内降级判定**（`agent.step` 失败 → `port.call router.select` → 按返回端口名再 `port.call` 备选；反向调用的 port 是运行时字符串、按 #33 `pins` 解析——**term 无动态端口，#14 静态管道不接**）；别名清单形状已在本插件 DESIGN 冻结（出生即固定、避免日后被迫 fork） | 调用实现（**不改任何调用方 `pins` 指向**）/ 提供降级顺序表以外的东西 | pins 无（`select` 是纯判定，不调模型）；`<-` 33（pins：`router`，随 #33 装配；`model` pin 与备选别名 pin 声明在调用方 #33） |
...
| 37 | `mcp` | MCP 适配器（**双向**）：出站接外部 MCP 服务器并把工具清单写成本身份投影（由 #14 入口 term 读出随 bag 传 #27；工具名 `mcp.<server>.<tool>` 命名空间化）；入站把本产品能力以 MCP 暴露（HTTP/SSE，**经 #15 主端口反代 `/p/mcp/*`**；外部请求 → `forward` 帧 → 宿主 run → **结果经 forward 的 `result` 同步回传**）；**外部进程不经 #25（保留例外；服务器清单条目须 `confirmed:true`（用户确认）才 spawn，`tools/list_changed` 只置脏标 + 下一拍 periodic 同步）**；出站发现 = 服务方法 `discover` + 宿主 periodic（reads 注入清单），产投影写计划 | 工具派发本体（归 27）/ 绕开 27 / 判定 / 直写世界 / 未经确认 spawn 外部进程 | pins 无；`<-` 27（pins：派发其工具）；**宿主定时触发**（`mcp.discover`，周期住 schema） |
...
| 40 | `ui-composer` | 输入卡（**composer 槽**）：文本 / **附件（原生选择器 + 任意格式）** / 模型 / 推理强度 / **权限档（全局，驱动 #25 强制）** / 发送终止 / **上下文用量行** | 消息流 / 列表 / 设置 / 判定 / 持有回合状态 | 无 pins；`~` 1（`input.read`，写槽前读当前 `slots` 做读-改-写）、2、14 / 33（`chat.send`）、`model.profile`（按名，归 #17 声明；消费者已在 #17 登记）；收 `context.assembled` / 宿主 `run.*`；**订阅 `api.uiState.active_thread`——发送写 `slots[active_thread]` 并给指令 / 命令携带 `thread=active_thread`**；写 = 直写 2 + `input` 槽；`<-` 15（挂载 composer）；版本提升：要求 18 移出输入卡 |
...
| 42 | `plugin-admin` | agent 的插件管理面：**工具名 `plugin.list` / `plugin.read` / `plugin.validate` / `plugin.write`**（产出写计划）；**可见性过滤**排除 `sandbox` 与自己；**受保护 `pins` 不可删**；`validate` 走宿主**入世 dry-run 面**，`list` 走宿主 `host.identities`；**`result_hash` 缓存住宿主 ③（键 = 候选树哈希），`write` 机械比对缓存**（不靠模型跨调用带回） | 改内核 / 改宿主 / 绕过过滤 / 装配换代 / 直接写链 / 人可见插件页（归 #17 S9）/ **改 #33 图数据（归 #45）** | `->` `host`（pins：`identities` / `source.read` / `validate_package`）；`<-` 27（pins：以工具类 `plugin-admin` 暴露）；读源码走宿主源码读面 |
...
| 44 | `evolve-metrics`（Rust） | **指标层**（纯计算、非 LLM、同输入同输出）：按 `(RefusalCode, attributable_to, workspace_id)` 聚类 + 成本异常 + 实例漂移 + 折叠候选 + `no_progress` + `post_failure` + `verify_failure` → 产证据条目计划；**`shadow` 影子回放（门禁第二道，零 token；配对源 = #43 `trace.eff_log`）**；**`record` 产 `user_request` 证据（`user_message_def` 由 #33 在派发时注入）**；**发 `orchestration.unhealthy` 事件**（**`aggregate` 亦为宿主周期方法**——图坏时仍能周期发，非 #17） | **提案**（分签红线）/ 调模型 / 直接写链 / 判定该不该改 | pins `host`（`audit`，影子回放读 `EffectAudit` 历史口径）；`+` 43、33（`thresholds` / trace 窗口：周期路径经 `schema.periodic.reads` 注入、#33 路径由其 bag 传，服务不读投影）；`<-` 33（pins：`aggregate`）、27（pins：**能力类工具绑定** `record` → `evolve-metrics`，2026-09-19 补）；**`sweep`/`aggregate` 两拍均宿主定时触发**（周期住 schema） |
| 45 | `orchestration-admin` | **agent 面编排管理**：**工具名 `orchestration.list` / `orchestration.read` / `orchestration.validate` / `orchestration.propose`**（`validate` = 本地复刻 #33 机械闸 dry-run，**输入由 #33 从自身 bag / 投影 `pins` 装配传入**；validate 只是预检、不构成门禁证据）；**只产提案条目、不产写**（提案计划补齐候选图 def 的 `put`）；**user_request 证据由 #44 `record` 产**（保分签）；**propose 只写台账、不入人闸——人闸在采纳**（`orchestration_change` 由 #33 提案扫描入 #32） | **产证据**（分签红线）/ 直接写图 / 改插件源码（归 #42）/ 装配换代 | pins 无；`+` 33、43、11 / 41（**由 #33 装配视图随 args 传入**，服务不读投影）；`<-` 27（pins：以工具类 `orchestration-admin` 暴露） |
| 46 | `ui-threads` | **线程顶栏**（slot `topbar`）：hover 显示 / 离开隐藏，位于侧边栏右侧、main 顶部；标签 = 线程列表（对话 → 会话标题 / 子代理 / 群聊 / 工作流）+ **待办标签位**（**按当前父会话读 #47**），点击切线程（经 `api.uiState` 广播 `active_thread`）；**内容按父会话隔离**；只读命令 `threads.state`；**侧栏 `session.select` 落账后收 `thread.updated` 重算并按 `current` 重置 `active_thread`**（单桥：`current` ↔ `active_thread`） | 对话渲染（归 #18）/ 群聊与步骤卡（归 #18 按 kind 分派）/ 判定 / 写世界本体 | 无 pins；`~` 14（读会话列表 / 标题）；`+` 47（待办标签，按父会话）、11（线程字段）；`<-` 15（挂载 topbar）；收事件 `thread.*` / `workflow.step` / `group.message`（emitter = #11；**运行 / 待审批角标同源 `thread.updated` 的 `status` / `pending`**，不另订宿主事件） |
...
| 49 | `session-title` | **会话自动标题**：按**首条用户消息**用**用户配置的模型**生成 **≤10 字**标题，写入 #11 `title`；**非流式**（`#12 model.complete`）、旁路失败不阻塞回合、不覆盖用户手动标题 | 标题显示（归 16 / 46）/ 重命名 UI（归 16）/ 调模型实现（归 12）/ 判定是否首条（归 14） | `->` 12（pins：`model.complete`）、11（pins：`set_title`）；`+` 2（**vendor / model / params 由 #14 入口 term 读出随 title 段 args 传入**，服务不读投影）；`<-` 14（pins：管道段 `session-title.generate`，**旁路段、#33 替换 pipeline 后仍由 #14 入口 term 保留**，仅首条消息） |
...
| 波 | 建什么 | 前置（pins） | 说明 |
| --- | --- | --- | --- |
| W0 基础 | #1 input、#2 config、#3 short-memory、#4–10 vendor-*、#11 session、#13 context-window、#15 ui-shell、#18 ui-chat、#20 embedding、#24 secrets、#25 sandbox、#26 guard、#35 agents、#36 skill、#37 mcp、#38 ui-notify、#41 workspace、#42 plugin-admin、#43 evolution | 无插件间 pins | 数据身份 + 无依赖执行件，可全部并行入世；ui-* 的 `~` 不构成建造前置。#15/#42 另 pin `host`（保留身份、非插件间依赖——headless 源码读面 / identities·validate 面） |
| W1 协议 / 工具 | #12 model-protocol（→24）、#28 tool-fs（→25、pin `host`）、#29 tool-shell（→24、25）、#30 tool-http（→25、pin `host`）、#31 tool-browser（→25、pin `host`） | W0 的 24 / 25 | 只有 #29 pin 密钥；#28/#30/#31 的 `host` pin 供二进制资产面（保留身份、非插件间依赖） |
| W2 配置 / 压缩 / 路由 | #17 ui-settings（→12、→24）、#19 compress（→12、→20）、#34 router（pins 无） | W1 的 12；W0 的 20 / 24 | #17 v1 pin 12 + 24（S8 `secrets.list`，入口命令 `secrets.status`）；S12 的 22/23 pins 等 W3 升代。#19 另 pin #20（去重向量化，执行件调用）。**#34 调用方 = #33（W6）：本波入世冻结声明与别名清单 schema 形状，验收（降级链）随 #33**；`router` pin 与备选别名 pin 均声明在 #33，#14 静态管道不接 |
| W3 记忆 / 工具注册表 | #21 memory-store（→20）、#22 memory-retrieval（→20、21）、#23 memory-consolidate（→19、20、21）、#45 orchestration-admin（无 pins）、#47 todo（无 pins）、#48 question（无 pins）、#27 tools（→26、28–31、37、42、45、47、48、11、19、21–23，另 pin `host`——`subagent.resume/terminate` 转交） | W0 的 20、42、43；W1 的 28–31、37；W2 的 19；本波 21–23、45、47、48 | #21 先于 #22 / #23；#27 最后。#17 在本波升一代（S12）。**#44 的 `record` 经 #27 暴露（#27 pin `evolve-metrics`），但 #44 在 W6 才建 ⇒ 该 pin 随 #33 通道（W6）补上，W3 的 #27 先不带此 pin** |
...
| W6 编排 | #44 evolve-metrics（pins `host`；**#27 本波补 `evolve-metrics` 绑定**）、#33 loop-policy（**execute + `implements ["loop-policy"]`**；→11、12、13、22、26、27、32、34、44） | W2–W4（22/23/34 在 W2/W3）；#44 先于 #33 | #33 替换 #14 管道是版本提升（此后 **#14 `pins` 增 `loop-policy`**，入口 term eff `loop-policy.interpret`；**title 旁路段保留归 #14**，`chat.resume` 续跑命令亦归 #14）；**#33 的宿主前置 = H1 / H2 / H5 / H6 / H12 / H14 / H16（`env` 注入）/ H17（`interpret` 方法级超时）**（#42 的 H13 不在此波，见 §1.13） |
...
> - **按队列项游标触发新 run（审批 / 提问续跑，H5）**——`#32` / `#48` 的 `enqueue` **会正常返回**，故那轮**不是 `waiting`**；正确形状是本 run 正常结束、`item` 带 resume 游标（`iter` / `cursor` / slots 引用），裁决 / 作答落账后**续跑（v1 机制见下「落地口径」，经 H18 / `chat.resume`）**。这与内核 `waiting` 续跑（同 `run_id` / 同 `directives` / 同 `now`）**不是同一机制**：内核续跑用于"效果未回灌"，审批 / 提问往返用于"人不在 30s 内"。
>   **v1 落地口径（2026-09-20，2026-09-20 收口）**：裁决 / 作答命令的入口 term（#39 `approval.decide` / `approval.decide_all`、#48 `question.answer`）直接产 `$directives` 计划——首条 eval 写 **`{kind:'eval', command:'chat.resume', args}`**（#14 声明的续跑命令：入口 term eff `loop-policy.interpret`、bag 带 resume 游标与裁决 / 作答结果；宿主按命令名解析入口 = **H18**，term 拿不到他人 def 哈希的问题由此消解），同批 `write` 记裁决 / 记答案 + 清槽；宿主既有 **plan 通道**在同一提交内起后续轮（新 `run_id`、投影取该轮轮首世界 ⇒ 能看到已落账的裁决），依赖 **H12 并发**。除 H18 外无需额外宿主代码；若 #32 / #48 实现时发现需要「非命令触发的自动续跑」，再补宿主触发面。
...
- **压缩引擎 = #19 `compress`**：`summarize`（→L1）/ `compact`（上下文压缩）/ `extract`（2–3 条 →L2）；`mode` 可纯算法（确定、零 token）或语义（调 #12）。**注意去重**。**自动触发唯一路径 = #13 75% 提示 → agent 经记忆工具调 #19**（无图节点、无 #33 回合尾维护——2026-09-20 与 #19/#23 三方统一）；#23 需要摘要时 eff `summarize` / `extract`。
- **自动压缩阈值**：#13 组装时按**可用预算 `budget = context_window - max_output - 余量`** 的 **75%** 判阈值（不是裸 `context_window`），达阈**追加一条 system 消息提示 agent 压缩**；agent 经记忆工具调 #19 落计划，`covered_upto` 前进后旧轮次不再进组装（摘要 + 后续消息 + 语义召回进）。**不丢图节点**（只压会话上下文，不动 #33 图 / 策略数据）。
- **维护 = #23 `memory-consolidate`**：L1 TTL 清理、L2 去重合并、L2→L3 固化、L3 遗忘 / 删除；**触发 = 宿主周期（H6，`schema.periodic`）**，不挂 #33 回合尾（2026-09-20 双方确认：#33 无回合尾维护节点，`-> 23` 旧口径作废）。**agent 顺带清理**：记忆 / 记忆维护工具目录暴露 **`memory.candidates` 具名工具**（绑 #23 `candidates`），agent 在查看 / 保存记忆的**同一回合**顺带调用它拿候选、当场决定删 / 合并（同回合语义保留；机制为显式工具调用，非复合工具）。**准则住 #33 策略数据**（何时压缩 / 记什么 / 忘什么）。
...
| H6 | ✅ **定时触发**：按插件 `schema` 顶层 `periodic` 声明（`{command\|method, every_ms, reads?}`）构造一次 run（命令按入口 term；方法直接调服务方法、其计划值由宿主落账）；**所需投影片段按 `schema.periodic.reads` 机械注入 bag**（服务不读投影） | #12 `sync`（方法）、**#23 `sweep` / `consolidate`、#32 `sweep`、#44 `sweep` / `aggregate`、#37 `mcp.discover`（方法）** | `host.md` §五 宿主扩展面 |
...
2. 再落 **H4 + H7 + H9 + H10 + H14**（③ 目录 / 密钥面 / 线程控制 / 事件 / 宿主保留能力类 `host`）——记忆族、UI 族、线程族的前置。
   > **进度（2026-09-20）**：**H4 / H7 / H9 / H10 / H14 已落地**（host 测试全绿 + 1 skip——各批次具体计数随提交演进、以 CI 为准，第三批后约 382 例、client 全绿）；`validate_package` 已由第三批 H13 落地（见下）。
...
| `session.new` / `select` / `rename` / `delete` / `restore` / `branch`（#16 命令） | `#1` 槽（`session.*` 槽体）+ `#11` 会话 body（select/delete 的 current 回退 / branch 源链） | eff args（#11 服务收） | #11 |
| `workspace.list` / `add` / `remove` / `reveal`（#16 命令） | `#1` 槽体 + `#41` body（add/remove 须当前列表做合并去重；reveal 解析 path） | eff args（#41 服务收） | #41 |
| `approval.decide` / `decide_all`（#39 命令） | `#1` 槽体 + `#32` item（含 `thread` / `resume`） | `{kind:'eval', command:'chat.resume', args}` 续跑计划 + `write` 记裁决 + 清槽（H18） | 宿主 plan 通道 → #14 → #33 |
| `question.answer`（#48 命令） | `#1` 槽体 + `#48` item | `{kind:'eval', command:'chat.resume', args:{cursor, thread, payload:{answers}}}` + `write` 记答案（标 `answered`）+ 清槽（H18；args 形状 = #14 契约） | 同上 |
| `threads.state`（#46） / `notify.state`（#38） | `#11` 线程字段 / `#47` 待办 / `#2` `ui.notify` 开关 | 入口 term 直出（不经服务） | 客户端 |
| `input.read`（#1 命令，新增） | `#1` body（整份 slots） | 直出（读命令带 args `{thread?}` 合法） | #40（写槽前读-改-写）、客户端 |
| `memory.view`（#17 读命令，W3 升代） | `#3` body、`#21` body + refs | eff args（#23 服务收） | #23 |
| `memory.edit`（#17 **写类命令，无参**，W3 升代） | `#1` 槽（kind `memory.edit`，载荷先入槽）+ `#3` body、`#21` body + refs | 入口 term 读槽与投影 → eff args（#23 服务收） | #23 |
| #12 `sync`（方法）/ #23 `sweep`/`consolidate` / #44 `sweep`/`aggregate` / `mcp.discover`（宿主周期） | 宿主按 `schema.periodic.reads` 机械注入（非 term） | bag（`reads` 声明的投影字面路径） | 各服务 |

> 服务侧义务对齐：#12 的 `+ 2 / 4–10`、#13 的 `+ 1/2/3/11`、#19 的 `+ 3/11`、#22 的 `+ 3/11`、#23 的 `+ 3/11/21`、#33 的 `+ 33六类/35/36/41/43/47`、#45 的 `+ 33/43/11/41` 均由本表对应入口承接；**#21 自身无 `+`（其 body+refs 由调用方入口 term 读出传入）**；各插件 `DESIGN.md` 的依赖行注明「由 §1.14 对应入口装配」。
...
## 回合一轮时序（#1–#18 + #24/#41 + #39/#40 就位后；#33 接管后的形态见注）

```
1. ui-composer 调 command input.read（读当前 slots）→ 本地合并本线程键（slots[active_thread] = chat.message 槽体）→ 提交 write：batch【put(input.body=合并后整份) + add_gen(input)】（携带 thread=active_thread）
2. ui-composer 调用 command chat.send（无参，信封带 thread）
3. 宿主构造 eval(entry = chat 入口 term, ctx = 该轮轮首投影)；入口 term 按 §1.14 装配 bag（#1 槽 / #2 连接与 permission 档 / #3 记忆 / #11 会话 / 系统提示 / 工具 schema…）
4. eff(model.chat, bag) -> eff(session.commit, bag)（v1 静态管道；#33 接管后 = eff(loop-policy.interpret, bag)，节点经 #33 反向 port.call 派发，#33 侧每步记 eff_log）
   · model.chat 期间 event(model.delta)（载荷带 `run` / `thread`，服务自帧 env 读取）-> 宿主广播 -> 各 UI 插件 -> SSE -> 浏览器（UI **按 `thread` 过滤**，只消费当前视图线程）
4b. （**仅首条用户消息**，#14 入口 term 判 `title` 缺省且 `count==0`）title 旁路段：eff(session-title.generate, {conversation, first_message, vendor, model, params, title_default})
   · #49 eff 12 model.complete（非流式）-> eff 11 set_title -> 标题落账发 thread.updated；失败 on_fail=ignore，不影响本回合（**#33 接管后本段仍由 #14 入口 term 保留**，不经图）
5. session.commit / interpret 返回计划：{$directives:[ write(batch: session gen + trace + slot idle), extern{reply} ]}（各段计划由入口 term 机械合并上提——按段序合并 $directives 数组，非拼 JSON）
...
> **审批 / 提问往返（#33 + #32/#48 + #39 就位后）**：`interpret` 在 `approval.wait` / `question` 处**正常返回**（本 run 结束，item 带 `thread` 与 `resume:{command:'chat.resume', args}` 落世界）；用户裁决 / 作答 → #39 / #48 命令的入口 term 产 `[eval(command:'chat.resume', args 含裁决 / 答案与游标), write(记 + 清槽)]`（H18）→ 宿主 plan 通道起新 run → #14 `chat.resume` 入口 term eff `interpret`（bag 带 resume）恢复执行，answers 作为 question 工具结果回灌。
