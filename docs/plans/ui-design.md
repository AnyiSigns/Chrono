...
| 按钮内等待（获取模型 / 重试 / 发送中 / 导出 / 添加工作目录的「等待选择…」） | 按钮 disabled + 16px **纯 opacity 呼吸环**（圆环描边 1.6s 25%↔60% 呼吸，**不旋转**——环形态即此，2026-09-20 修订，废除旧「spinner 1s linear」口径）+ 文字进行式（「获取中…」），完成 150ms 恢复。**等待态必须配进行式文字**——与 disabled（静态 `.45`、无文字）的区分靠文字，不靠旋转 |
...
- headless 前端**不进挂载表**、不占端口、不被 shell 反代（**不 mount 任何 slot**）；但仍有**浏览器侧入口 bundle**（`entry.js`，住插件包 `web/`，如 `web/entry.js`）——由 shell 的**独立 headless 清单**（`state/ui-headless.json` = `[{id, entry}]`，③ 可重算）加载（不进 `state/ui-mounts.json`），用于在浏览器侧调 `Notification` API 等前端能力（宿主侧服务调不了浏览器 API）。加载路径 = shell 提供（**同源静态，不经 `/p/` 反代、不占 slot**）：shell 按清单的 `{id, entry}`（`entry` = 插件包内路径，默认 `web/entry.js`）经**宿主「插件源码读面」`host.source.read`**（`host.md` §五 宿主扩展面；H3 已落地；shell pin `host`）取字节并同源服务。
...
| `composer` | 40 `ui-composer` | 底部输入区，自动增高、**≤30vh 超出内滚**（2026-09-20 定）；固定在 column 底部 |
...
- **纵向空间竞争（写死，2026-09-20 补）**：column 高度 = topbar（0，overlay 不占位）+ main（弹性，**`min-height: 120px` 保底**）+ dock（≤40vh、超出内滚）+ composer（≤30vh、超出内滚）。dock 与 composer 同时打满仍余 main ≥30vh；极矮窗口空间不足时**让步顺序：dock 先缩（本就内滚、条目默认单行摘要）→ composer 再缩（内滚不丢草稿）→ main 保底不压没**——裁决时始终能看到消息流上下文。
...
- **打开**：焦点移入首个可操作项（或弹层容器）；**单选下拉例外**：初始 `aria-activedescendant` = **当前选中项**（无选中项退首条），`↑/↓` 从选中项起移动（2026-09-20 定，与原生 select 习惯一致）。
...
- **同层（toast / lightbox 均 70）以 DOM 序兜底**：toast 由壳挂 `<body>` 末端、恒晚于子应用内的 lightbox ⇒ **toast 在 lightbox 之上可见**（2026-09-20 写死，不为此新增层级值）。
