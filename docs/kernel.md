...
它只兑现三个承诺：**改坏了能退**（日志只追加、回滚是一次追加而非抹除）、**非法进不来**（世界写口前的四步机械校验任一不过则整次调用作废）、**历史改不掉**（链式哈希覆盖每条记录的位置与内容摘要，改任何一处其后全断）。它明确不兑现第四个：不判断"这次改动是不是更好"。判决回答的是"合不合法"，正当性（该不该）永远在上层。

**三个承诺的作用域 = 定义平面**。它们是对"进了世界的东西"说的，而进世界的只有**定义与判定**：插件代码世代、声明、`pins`、绑定、阈值、以及采纳闸与回滚要读的证据与判决。系统运行中产出的记录不进世界，故不受这三条承诺覆盖，也不该为换取这三条承诺而被写成世代（判据与理由见 §八）。内核对此不设机制：它只给"被 `put` 进来的东西"做版本，**什么配进世界是载体与插件的判定**，不是内核的动词。
...
| 宿主 | 世界落盘、效果执行、装载执行件、审计策略、上下文投影、给插件划持久地盘 | 判定逻辑（那是数据）、插件持久存储的内容 |
| 执行件（插件） | 干活，独立进程、按协议挂载；自有运行数据由自己持久化 | 世界状态的所有权 |
| 上层 | 用内核给的一致性做自己的判定 | — |

**"世界状态的所有权"不等于"数据的所有权"**：插件不拥有世界状态（改世界只能提交内容、经 M4 的门），但它**拥有自己的运行数据**——存哪、用什么引擎、怎么建索引、怎么做事务，全归插件；载体只保证那块地盘的位置、跨代存活、备份归属与跨代独占，不解释其内容、不定义其形态。载体是中转站：转发调用、解析 `pins`、装配依赖、划地盘，**不认识插件是什么**。
...
| M2 归约机 | 14 原语、可执行的 def、显式效果挂起 | 判定逻辑只能编进宿主 ⇒ 改判据要重启、不可回滚、不留痕 |
| M3 日志 | 只追加、链式哈希、位置 CAS | 无法回滚、审计、重放 ⇒ 内核存在的唯一理由消失 |
| M4 世界写口 | **改世界**的所有动作穿过同一道门，四步机械校验 | "非法进不来"变成靠自觉 |

M4 是**世界这一处的门**，不是系统的总门。它说的是"改定义平面只有一条路"，不是"系统里所有持久化只有一条路"——后者从来不成立（审计侧存、CAS 字节、本地密钥、运维日志、插件自有存储各有其处，落点表见 `host.md` §五「写入落点」）。载体是中转站而非总账房：它在世界那一处是独占写者，因为 `journal` 是单链 + `expect_pos` CAS，两个写者并发追加会当场打断链哈希——这是物理约束，不是地位。

插件写自己的运行数据不经这道门，也不需要：那不是共享状态，纪律是**每份运行数据只有一个 owner 身份**（owner 自己写，别人经能力调用问它），不是全局串行。把运行数据塞进这道门换不来更强的一致性，只会让单写者锁与回收窗口去争本不该它们管的东西。
...
**运行记录不占四档**：四档划的是"世界内容"的档位，四条判据都以"影响世界现状、需被引用 / pin"为前提。插件运行中产出的记录——对话消息、输入槽、审批队列与结果、工具结果、待办、界面配置、记忆条目——不被引用、不被 pin、回滚时不该跟着回退、判定与门禁不必从世界读它。它**根本不进世界**，故既不占 ①②，也不是 ③④ 那种"世界留哈希声明、本体外迁"的形态：世界里连声明都没有。它住插件自有持久存储，由载体划地盘、插件选引擎，内核看不见。判据两问：**回滚该不该带上它**、**判定 / 门禁 / 重放要不要从世界读它**；两问皆否即出世界。例外按同一判据留下——采纳闸与回滚要读的证据、判决、阈值、绑定仍是 ① 声明。

把运行记录当 ① 写的代价是结构性的，不是量级问题：每改一次就是一个新 def、一个新世代、一个新 `worldRev`、一份回收窗口占用。内容寻址服务于**不可变声明**是资产，服务于**可变记录**是负债——后者要的是事务与查询，不是版本链。
...
十四个原语（term 语法，`kernel.md` §12.1）：

```
Const ["c", Json]                  Get   ["g", Path]            Var  ["v", int]
GetAt ["get", T, Path]             GetOr ["getOr", T, Path, T]  Cmp  ["cmp", T, T]→-1|0|1
Pred  ["pred", op, T, T]→Bool      If    ["if", T, T, T]         Fold ["fold", coll, init, step]
Eff   ["eff", port, method, args]  Call  ["call", fTerm, [T...]]
Arith ["arith", op, T, T]          List  ["list", [T...]]        Obj  ["obj", {k: T...}]
```

`pred`（op ∈ lt/le/gt/ge/eq/ne）复用 `cmp` 全序，是 `if` 唯一可计算出来的布尔来源；`get` 对任意值沿静态 path 投影，与 `g` 对称（`g` 的根是 `ctx`，`get` 的根是任意值）；`getOr` 同 `get` 但缺失路径时惰性取默认项，使外部数据里的**可选字段**可在 term 内安全读取。`arith`（op ∈ add/sub/mul）只接受有限数、结果非有限报 `bad_arith`；`list`/`obj` 逐个求值构造新列表/新对象（`obj` 键按 code-unit 升序，与 `canonicalJson`/`cmp` 同序）。**先不加 `div`**：除零与非终止小数语义复杂、收益低。这些全是**加法式扩展**（原语只增不改语义）：不改身份、终止性与链格式，老日志不含新原语仍可重放；新原语在旧实现上报 `bad_term` 而非算错。

边界随之外移一格：term 从"只能选择 / 投影"扩到"可推导纯数值、可组装结果对象"，但**重计算与效果仍归执行件**——判定管选择、编排与轻量组装，数值聚合之外的重活不进 term。
...
| 世界的第二个写口 | M4 全部价值在于**改世界**只有一个门（其它落点自有其写者，不是"第二个世界写口"） |
| 多链合并 | §十四边界 5 |
