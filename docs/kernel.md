...
| | 是什么 | 产物 | 与内核的关系 |
|---|---|---|---|
| 研究 | 在纯程序化的假世界里证明"某个进化假设成立" | 可执行验收器、预注册阈值、失败模式 | 旁线：只能以**数据**身份进入 |
| **内核** | 可复现的世界 + 谱系 + 机械校验 | 世界版本、世代链、判决、观测流 | 本文件 |
| 产品 | 给人用的面 | 用户体验 | 下游：全部是世界的投影 |

一句话：**研究管"怎么测"，产品管"测什么"，内核管"测出来之后改什么、怎么改、改坏了怎么退"。**

## 0.2 三条单向

1. **依赖单向**：产品 → 载体 → 内核；内核**零依赖**，研究不在链上。
2. **真源单向**：内核的（世界 + 日志）是**唯一真源**；上层的配置、UI 布局、命令面、宿主清单全部是**投影**——不落盘、不生成、不需要门禁守漂移。
3. **判定单向**：验收器与阈值只能以**数据**身份进入世界，不能以代码身份进内核。

## 0.3 内核之外有什么

图 · 作用域 · 通道 · 路由策略 · 提示词 · 参数 · 权重 · 基因组 · 工具绑定 · UI 描述 · 可见性规则 · 判定标准 —— 全部是**世界内容**。内核不知道里面有什么，只要求它们有身份、有世代、能标注依附。

一份内容进不进样本、算不算可训，由上层按**稳定性**自行分类（可训 / 冻结叶子 / 检索绑定 / 可搜索）。**内核只要求"每个身份声明一个 class 引用"，不认识这些类的语义。**
...
1. 第一方插件必须声明**和第三方一样的字段**：`schema` / `pins` / `sig` / 度量契约 / 能力需求。**想给第一方开后门时，说明接口缺了一样东西——补接口，不要开洞。**
...
2. **平层让 §6 的依赖 DAG 一眼可见**；嵌套会把依赖藏进路径（`layout.md` §7 的判据：分类若能用数据表达，就不要用目录表达）。
...
`compact` 与回收：见 10.7（**P2 之后再做**，出口留在本文件）。
...
- `sig` 与 `pins` **双缺** → 恒 `false`：没有任何绑定的 def 不会随世代更替而失效。这是**宽松**口径——要让样本按绑定被隔离，`pins` 必须显式声明（S3 的证据约定、S7 的样本过滤都依赖这一点）。
...
- **续跑契约**：`waiting` 之后必须以**同一 `run_id` + 同一份完整 `directives`（含已执行完的前缀）+ 同一 `now`** 重调，`results` 只增不改。`i` 是 directive 下标，`now` 进 `entryHash`——任一漂移都会让续跑与"一次跑完"不再逐字节一致（S1 判据）。
...
P2 之后才谈 §10.7 的 `compact` 与回收（出口留在本文件）。
