...
- **算术 / 数据构造：当前原语没有，属范围取舍，不是原则禁令。** 原语不能对数值做加减乘除，也不能构造新的列表/对象（新值只来自 `c` 字面量与 `eff` 回灌）。内核**可以**加（「原语只增不改语义」，向后兼容，老实现遇新原语报 `bad_term` 而非算错）；是否加是产品 / 设计决定。在未加之前，工具链不提供对应语法（不可降级），数值计算与结构构造归执行件（经 `eff`）。
...
- 暂不提供算术 / 数据构造语法（当前原语不具备，见 §四；若要，属一次内核原语扩展，单独立项，工具链随裁决扩展降级规则）。
