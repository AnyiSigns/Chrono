// 包形状测试：零 schema、members = execute + term、无 pins、命令入口 term 形状、
...
test('能力类为 ui-threads（ping 占位 + threads.state 装配方法）；无 pins', () => {
  const decl = readJson('plugin.json')
  assert.deepEqual(decl.implements, ['ui-threads'])
  assert.deepEqual(decl.methods, { 'ui-threads': ['ping', 'threads.state'] })
  assert.deepEqual(decl.pins, {})
})

test('members = execute + term；命令入口 term 存在且无参', () => {
  const decl = readJson('plugin.json')
  assert.deepEqual(decl.members, [
    { kind: 'execute', path: 'execute/' },
    { kind: 'term', path: 'terms/' },
  ])
  assert.deepEqual(decl.commands.map((command) => command.name), ['threads.state'])
...
test('入口 term 形状：只传投影切片 `ctx.ids` 给本插件服务', () => {
