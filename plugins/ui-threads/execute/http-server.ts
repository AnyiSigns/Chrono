// 本插件子应用 HTTP 服务：静态浏览器模块 + 自己的 `/events` SSE + 只读命令入站桥。
// 只做转译与静态服务，不认识业务；绑定 127.0.0.1。浏览器不直连本端口，壳反代 `/p/ui-threads/*`。

import { createServer } from 'node:http'
import type { IncomingMessage, Server, ServerResponse, Socket } from 'node:http'
import { Bridge } from './bridge.ts'
import { connectionStateRecord, SseHub, THREADS_IMPL } from './events.ts'
...
function handleEvents(deps: UiServerDeps, req: IncomingMessage, res: ServerResponse): void {
  ...
  res.write(': connected\n\n')
  deps.sse.add(res)
  res.write(`data: ${JSON.stringify(connectionStateRecord(deps.connected()))}\n\n`)
  const cleanup = (): void => {
    deps.sse.remove(res)
  }
  ...
  try {
    switch (route.kind) {
      ...
      case 'api-command':
        ...
        return
      case 'api-state':
        sendJson(res, 200, { ok: true, connected: deps.connected(), impl: THREADS_IMPL })
        return
      default:
