...
// 默认值与壳的挂载表默认值一致（`ui-threads -> 8793`）；两侧各自读同一环境变量，保证装配一致。

/** 壳挂载表给 `ui-threads` 的默认端口。 */
export const DEFAULT_THREADS_PORT = 8793

/** 本插件端口覆盖环境变量名（ID 大写、非字母数字转 `_`）。 */
export const PORT_ENV_KEY = 'CHRONO_UI_PORT_UI_THREADS'
...
export function resolvePort(env: { [key: string]: string | undefined }): number {
  return (
    ...
    parsePort(env['CHRONO_UI_PORT_ui-threads']) ??
    DEFAULT_THREADS_PORT
  )
}
