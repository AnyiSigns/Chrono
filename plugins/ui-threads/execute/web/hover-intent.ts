...
// 只描述状态迁移与应排定的延时，计时器由调用方持有（便于单测）。纯模块：无 DOM、无 react import。
...
export type HoverStatus = 'hidden' | 'showing' | 'visible' | 'hiding'
export type HoverEvent = 'enter' | 'leave' | 'timeout'

/** 状态取值：`hidden` / `showing` / `visible` / `hiding`。 */
export const HOVER_STATUSES: HoverStatus[] = ['hidden', 'showing', 'visible', 'hiding']

/**
 * 迁移一次：`event` ∈ `enter` / `leave` / `timeout`。
 * 非法状态 / 事件回原状态（不抛错）。
 */
export function nextHoverStatus(status: string, event: string): HoverStatus {
  if (!HOVER_STATUSES.includes(status as HoverStatus)) return 'hidden'
...
export function hoverDelay(status: HoverStatus | string): number | null {
...
/** 该状态是否展开（overlay 可见）。 */
export function isHoverOpen(status: HoverStatus | string): boolean {
