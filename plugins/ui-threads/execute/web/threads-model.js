// 线程树与角标的纯逻辑（无 DOM、无 IO、无文案）：服务侧装配与浏览器侧渲染共用。
...
export function conversationKind(conversation) {
  ...
  return THREAD_KINDS.includes(kind) ? kind : 'main'
}

/** 父线程 id：`parent.def` 为字符串时返回它，否则 null。 */
export function parentIdOf(conversation) {
...
export function isDeleted(conversation) {
...
export function activeConversations(conversations) {
  ...
  return conversations.filter((item) => isRecord(item) && !isDeleted(item))
}

function indexById(conversations) {
  const byId = new Map()
...
function firstMainId(conversations) {
  for (const conversation of conversations) {
    if (conversationKind(conversation) === 'main') return conversation.id
  }
 ...
 */
export function resolveRootMainId(conversations, current) {
  const list = activeConversations(conversations)
  const byId = indexById(list)
  let id = typeof current === 'string' && byId.has(current) ? current : firstMainId(list)
 ...
 */
export function orderSubtree(conversations, rootId) {
  ...
  for (const conversation of list) {
    ...
    childrenOf.get(parent).push(conversation.id)
  }
  const ordered = []
  const seen = new Set()
  const visit = (id) => {
...
export function pendingOf(conversation) {
 ...
 */
export function badgeOf(conversation) {
...
/** 文案键：按种类取标签兜底（真实文案住 messages.js，本模块不带文案）。 */
export function threadLabelKey(kind) {
  return THREAD_KINDS.includes(kind) ? `thread_label_${kind}` : 'thread_label_main'
}
