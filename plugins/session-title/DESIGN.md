...
| 依赖 | `->` 12（pins：`model.complete`，非流式单次补全）、`->` 11（pins：`set_title`）；`+` 2（读所选 vendor / model / params）；`<-` 14（pins：管道段 `session-title.generate`，仅首条用户消息触发） |
| 成员 | execute, schema |
...
- 由 **`#14 chat` 管道**在**首条用户消息**的回合追加一段 `session-title.generate`：
  - `when = first_message`：`#14` term 读 `#11` 投影，会话 `title` 仍为缺省「新对话」且 `count == 0`（即本回合是首条用户消息）时触发；否则**跳过**。
  - args = `{ conversation, first_message }`（`first_message` = 本回合用户消息文本，由 `#14` 从 `#1` 槽读出）。
- **旁路失败（写死）**：`on_fail = "ignore"`——标题生成失败**不使主回合 `refused`**（标题是旁路增强，不是回合的一部分）。

## 生成

- 读 `+ 2 config`：`vendor` / 所选 `model` / `params`——**用户配置的模型**，本插件不内置任何模型名、不自建默认表。
...
- **不覆盖用户手动标题**：仅当 `title` 仍为缺省值时写入；用户经 `#16` 重命名过则跳过（`#14` 的 `when` 已判，本插件**写入前再校验一次**，防竞态）。

## 写入

- `eff 11 set_title { conversation, title }` → `#11` 返回写计划（`put` 会话 body 的 `title` + `add_gen`）；**可回放**。
- `#11` 落账后机械发 `thread.updated`（标题变）→ `#16` / `#46` 自动跟随，**无需新事件**。

## 跨插件登记

- **#12 model-protocol（版本提升：被提升方）**：新增方法 **`model.complete`**（非流式单次补全；同 `chat` 的密钥 / 韧性路径，**不发 `model.delta`**）。
- **#2 config（跨插件登记）**：本插件 `+ 2` 读所选 `vendor` / `model` / `params`；#2 的 `<-` 登记加 49。
- **#11 session（版本提升：被提升方）**：新增方法 **`set_title {conversation, title}`**（服务调用路径，区别于读槽的 `rename`）。
- **#14 chat（版本提升：被提升方）**：管道新增 `session-title.generate` 段（`when = first_message`、`on_fail = ignore`）；新增 pin **`session-title`**。
