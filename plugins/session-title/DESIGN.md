...
- 由 **`#14` 入口 term** 判定（`#11` 投影 `title` 缺省且 `count == 0`，轮首 round-anchored）后**在首条用户消息的回合追加 title 段** `session-title.generate`（2026-09-20 修订）：
  ...
  - args = `{ conversation, first_message, vendor, model, params, config, session, title_default }`（#14 入口 term 读 `#1` / `#2` / `#11` 装配，§1.14；`first_message` = 本回合用户消息文本）。**`config`（连接实例 + quirks + 模型 + 参数）与 `session`（会话 body）必须装配传入**——`generate` 以 `args.config` 走 #12 `model.complete`、以 `args.session` 走 #11 `set_title`（本插件已支持两字段透传，2026-09-21 修订）。
...
- `vendor` / 所选 `model` / `params` / `config`（连接实例 + quirks）/ `session`（会话 body）**由 #14 入口 term 读出随 title 段 args 传入**（§1.14；服务不读投影）——**用户配置的模型**，本插件不内置任何模型名、不自建默认表。（2026-09-21 修订）
...
- **#14 chat（版本提升：被提升方）**：管道新增 `session-title.generate` 段（`when = first_message`、`on_fail = ignore`）；新增 pin **`session-title`**；**title 段 args 扩展为 `{conversation, first_message, vendor, model, params, config, session, title_default}`（#14 入口 term 装配，§1.14）——`config` = 连接实例 + quirks + 模型 + 参数，`session` = 会话 body，二者必须装配传入，否则本插件 `set_title` / `model.complete` 真实链路无法工作（§1.6 跨插件登记：调用方装配口径）**；旁路段在 #33 接管后保留归 #14（2026-09-21 修订）。
