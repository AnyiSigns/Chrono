...
- **布尔 `reasoning_map` 的口径**：本家 `reasoning_field` 是开关式（`enable_thinking`），`low` / `medium` / `high` 三档**塌缩为同一个布尔值**（`true` = 开思考）——#40 的三档控件对同一开关显示同一效果；**#40 在 `reasoning_map` 值全同时折叠档位控件为单开关（防假档位）**（2026-09-20 修订）；若要区分思考预算，须映射到厂商真实预算字段（v1 无）。
