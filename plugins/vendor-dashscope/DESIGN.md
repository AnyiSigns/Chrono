# #6 `vendor-dashscope`（厂商适配）

| 字段 | 内容 |
| --- | --- |
| 编号 / 身份 | 6 / `vendor-dashscope` |
...
| 状态 | 细节设计（2026-09-19）：`quirks` 冻结；**字段值待核对** |

```jsonc
{ "name": "DashScope",
  "sdk": "dashscope",
  "default_base_url": "https://dashscope.aliyuncs.com/compatible-mode/v1",
  "default_auth_ref_name": "DASHSCOPE_API_KEY",
  ...
  "quirks": {
    ...
    "reasoning_field": "enable_thinking",               // Qwen3 开关式（待核对）
    ...
    "note": "compatible-mode（OpenAI 兼容）；思考开关式" } }
```

- 规范字段含义见 `#4 vendor-openai`；本文件只填厂商值。
- **布尔 `reasoning_map` 的口径**：本家 `reasoning_field` 是开关式（`enable_thinking`），`low` / `medium` / `high` 三档**塌缩为同一个布尔值**（`true` = 开思考）——#40 的三档控件对同一开关显示同一效果；若要区分思考预算，须映射到厂商真实预算字段（v1 无）。
