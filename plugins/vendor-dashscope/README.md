# vendor-dashscope（DashScope 厂商适配模板）
...
- 推理：`enable_thinking` 布尔开关，归一三档塌缩为同一个 `true`（开思考）；无档位区分。
- 最大输出字段：`max_tokens`。
...
| 项 | 取值 | 核对 |
| --- | --- | --- |
| base_url | `https://dashscope.aliyuncs.com/compatible-mode/v1` | 2026-09 官方文档确认（国际站另有 `dashscope-intl` 端点） |
| auth_ref_name | `DASHSCOPE_API_KEY` | 2026-09 官方文档确认 |
...
无命令。数据由写入方的计划落账，读取方按字面身份名读投影（`ids.vendor-dashscope.body`）。
...
```
node plugins/vendor-dashscope/tools/seed-default-body.mjs --root <宿主根目录>
```
