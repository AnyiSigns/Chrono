// 反向调用通道（服务 → 宿主，docs/protocol.md §2.4）：`pins` 含 `host` → host，
// 用于按需解析投影引用（`host.def.read`，只读）。失败作数据，不抛未捕获错误、不断通道。

import { writeFrame } from './frames.ts'
import type { Json, Rec } from './plan.ts'
...
/** 一条服务连接上的反向调用登记表：`call` 发 `port.call` 并等待应答，帧循环收到应答时调 `settle`。 */
