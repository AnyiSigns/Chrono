...
| 依赖 | pins 无；`<-` 12（读 `sdk` 与 `quirks`）、17（**间接**（经 #12 `model.vendors` 读模板））（2026-09-20 修订） |
| 成员 | schema |
| 能力类·方法 / 命令 | 无 |
...
| 验收 | 1) 7 个同 schema；2) **数据身份可测**：schema 形状与 #4 一致 / 不含明文密钥 / 值类型合法（`reasoning_map` 值 ∈ `string \| number \| boolean`）；3) 换模板 / 加厂商不改 #12 代码（原「被 #12 机械解释 / 加厂商不改 #12」为 **#12 侧验收引用**）（2026-09-20 修订） |
| 状态 | 细节设计（2026-09-19）：**字段形状已冻结**；**厂商取值待核对（W0 建包前核对为显式前置任务）**（2026-09-20 修订）；**混合**实现（三协议自实现 + 个别厂商 SDK）；本文件为七家规范源 |
