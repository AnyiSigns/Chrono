...
- 标题 = 事件类型；正文 = 人类可读会话标签（载荷 `label`，缺失则省略）+ 首行摘要，
  整体按 **80 码点**截断。原始 `thread` id 不作为标签。
- 点击通知聚焦 shell。
- 构造失败（参数非法 / 平台拒绝）时回收已受理的同屏配额，异常不外泄，后续通知不受影响。

## 去重与节流

- 同一 `(thread, kind)` 在 **5s 窗口**内只弹一条，后续到达**合并计数**（计数附在正文）。
- **同屏最多 3 条**，超出按到达顺序排队，关闭一条后弹出下一条。
- `question.pending` / `orchestration.unhealthy` 属**结构性事件**：即时弹、不占同屏配额，
  但仍按 5s 窗口对**同键**去重（合并计数），**异键**以同屏上限封顶（超出抑制），避免无界堆叠。
- `orchestration.unhealthy` 的 `thread:null` ⇒ 去重键退化为 `kind`。
- 去重窗口表按窗口期剪除，不随历史键无限增长。
- 勿扰 / 免打扰交系统策略，本插件不做。
...
- 自初始化流程：发布权限状态 → 取 `notify.state` 开关与文案表 → 连 `/events` →
  按规则弹通知；权限在事件到达时刷新，焦点随窗口 `focus` / `blur` 即时刷新。
