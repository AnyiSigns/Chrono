...
const HANDLERS = createHandlers({
  file: secretsFileFromEnv(process.env),
  env: process.env,
})

// 声明方法集：plugin.json 声明优先，缺声明回落处理器表（HANDLERS 必须先初始化，避免 TDZ）
...
function handleCall(message: Rec): void {
  ...
  try {
    ...
    if (err instanceof SecretError) {
      // 领域错误：只回码与不含值 / 文件内容的描述，不写 stderr
      sendError(id, err.code, err.message)
      return
    }
    log(`method ${method} failed`) // 不打印 args，防明文泄漏
    sendError(id, 'internal', 'handler failed')
    return
  }
  writeFrame({ v: '1', id, kind: 'result', ok: true, value })
}
