// `memory-consolidate` 服务进程入口：宿主服务协议帧循环（docs/protocol.md §二）。
// manifest 从同包 plugin.json 派生（服务自述与声明一致）；stdout 只发协议帧，日志走 stderr；
// stdin EOF / 管道断开即自退出。服务不读投影、无写通道；#3 / #11 / #21 数据由调用方入口 term
// 或宿主 periodic.reads 装配进 args。反向调用（embedding.chunk / embedding.embed / compress.summarize）
// 走 `port.call`，应答帧立即结算（不排队）。
