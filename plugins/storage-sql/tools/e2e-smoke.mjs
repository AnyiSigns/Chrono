// storage-sql 端到端冒烟（黑盒，经 boot CLI）：pack → seed → start → 核对声明 / 命令 / .worldignore
// → 核对宿主为 durable 身份建了 ④ 目录 → stop。任何失败路径都会尝试 stop 释放锁。
// 用法：node plugins/storage-sql/tools/e2e-smoke.mjs
...
const PKG_DIR = join(REPO_ROOT, 'plugins', 'storage-sql')
...
function main() {
  ...
  const root = join(tmpdir(), 'kilo', `chrono-storage-sql-${stamp}`)
  ...
  try {
    ...
    console.log('commands: storage-sql 无命令（符合声明）')
