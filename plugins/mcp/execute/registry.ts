...
export class McpRegistry {
  ...
  private readonly resolveAuthRef: (authRef: Rec) => Promise<SecretResult>

  constructor(
    log: (line: string) => void,
    sink: EventSink,
    resolveAuthRef: (authRef: Rec) => Promise<SecretResult>,
  ) {
    this.log = log
    this.sink = sink
   ...
   */
  async discover(bodyInput: Json | undefined): Promise<DiscoverOutcome> {
  ...
  async invoke(toolRef: string, toolArgs: Json): Promise<Json> {
    ...
    try {
      const conn = await this.ensureConnected(runtime)
      ...
      runtime.toolsSignature = ''
      runtime.failures += 1
      runtime.lastError = errMessage(err)
      if (runtime.failures >= runtime.restart.max) {
        runtime.isolated = true
        runtime.isolation = { reason: runtime.lastError, failures: runtime.failures }
        this.emitServer(runtime, 'isolated', { failures: runtime.failures, reason: runtime.lastError })
      }
      return errorValue('mcp_call_failed', errMessage(err))
    }
  }
  ...
  private async refresh(runtime: Runtime): Promise<void> {
    try {
      ...
      if (runtime.failures >= runtime.restart.max) {
        ...
        this.emitServer(runtime, 'connect_failed', {
  ...
  private async ensureConnected(runtime: Runtime): Promise<McpConnection> {
    if (runtime.conn !== null && runtime.conn.alive) return runtime.conn
    runtime.conn?.close()
    const env = await this.buildEnv(runtime.entry)
    const conn = new McpConnection(
      ...
      },
      {
        ...
        onDirty: () => undefined,
        onExit: (reason) => this.handleExit(runtime, reason),
      },
    )
    runtime.conn = conn
    await conn.connect()
   ...
   */
  private async buildEnv(entry: Rec): Promise<Record<string, string>> {
    ...
    for (const [key, value] of Object.entries(raw)) {
      ...
      const resolved = await this.resolveAuthRef(authRef)
      if (!resolved.ok) {
  ...
  /** 子进程退出：`never` 策略直接隔离；否则下一拍 discover 重连重拉。 */
  private handleExit(runtime: Runtime, reason: string): void {
    runtime.conn = null
    runtime.toolsSignature = ''
    if (runtime.restart.policy === 'never') {
      runtime.isolated = true
      runtime.isolation = { reason: `server_exited: ${reason}`, failures: runtime.failures }
      this.emitServer(runtime, 'isolated', { reason, failures: runtime.failures })
      return
    }
    this.emitServer(runtime, 'exited', { reason })
  }
