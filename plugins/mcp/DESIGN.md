...
- **触发方式（D6 / 入站转发）**：
  ...
  - **入站 = 宿主转发入站帧触发**：外部请求到 `/p/mcp/*` → 壳转成宿主入站帧 → **宿主转发到本服务** → 触发 `mcp.sync`（入站分支）跑一次 → 产命令+槽写计划 → 宿主落账并按槽触发 run（宿主待补能力「插件入站转发」，见下）。
...
- **#15 ui-shell**：入站面经主端口同源反代（`/p/mcp/*`）；壳转成宿主入站帧、**宿主转发到本服务触发 `mcp.sync`（入站分支）**（宿主待补能力「插件入站转发」）。
