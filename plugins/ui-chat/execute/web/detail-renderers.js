...
// 未知 kind → 文本降级（不空白、不报错）。DOM 构建在 entry.js，本模块只出可测数据。

import { assetSource, safeStringify } from './render-parts.js'
 ...
 */
export function computeDiff(before, after, context = 3) {
...
export function parsePatch(patch, context = 3) {
  const rows = []
  let oldLine = 0
  let newLine = 0
...
function normalizeStrings(value) {
  if (!Array.isArray(value)) return []
  return value.map((item) => (typeof item === 'string' ? item : isRec(item) && typeof item.path === 'string' ? item.path : safeStringify(item)))
}

function normalizeQuestion(question) {
  if (!isRec(question)) return { id: '', header: '', question: String(question ?? ''), options: [], multiple: false, custom: true }
...
function questionViewModel(detail) {
...
/** detail 描述符 → 渲染器视图模型。 */
export function detailViewModel(detail) {
  ...
  switch (kind) {
    ...
    case 'table': {
      const columns = Array.isArray(detail.columns) ? detail.columns.map((column) => (isRec(column) ? String(column.title ?? column.key ?? '') : String(column))) : []
      const rows = Array.isArray(detail.rows) ? detail.rows.map((row) => (Array.isArray(row) ? row.map(cellText) : [])) : []
