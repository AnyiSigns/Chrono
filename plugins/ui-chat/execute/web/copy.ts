...
export interface CopyState {
  status: 'idle' | 'check' | 'error'
  since: number
  revertAt: number | null
}

/** 创建复制反馈状态；`now` 可注入假时钟便于单测。 */
export function createCopyState(options: { now?: () => number } = {}): {
  get(): CopyState
  success(): CopyState
  fail(): CopyState
  reset(): CopyState
  tick(at?: number): CopyState
} {
  const now = typeof options.now === 'function' ? options.now : () => Date.now()
  let state: CopyState = { status: 'idle', since: 0, revertAt: null }
  const api = {
    get(): CopyState {
      return { ...state }
    },
