...
export function usageTotal(usage) {
  ...
  const total = firstNumber(usage, ['total_tokens', 'totalTokens', 'total'])
  if (total !== null) return total
  const input = firstNumber(usage, ['prompt_tokens', 'input_tokens', 'inputTokens'])
  const output = firstNumber(usage, ['completion_tokens', 'output_tokens', 'outputTokens'])
  if (input === null && output === null) return null
  return (input ?? 0) + (output ?? 0)
}

/** 消息 def → 脚注用量文案；无 `meta.usage` 返回 null。 */
export function usageText(def) {
