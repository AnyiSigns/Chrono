...
// 浏览器经壳反代 `/p/ui-chat/*` 到本端口；静态模块与 `/events`、`/api/*` 都在此判定。
...
export function routeOf(method: string, pathname: string): Route {
  ...
  if (pathname === '/api/asset') return verb === 'GET' ? { kind: 'api-asset-get' } : { kind: 'not-found' }
  ...
  if (pathname === '/api/question/answer') {
    return verb === 'POST' ? { kind: 'api-question-answer' } : { kind: 'not-found' }
  }
