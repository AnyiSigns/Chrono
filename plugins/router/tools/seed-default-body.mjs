// 预置 router 别名清单默认 body（数据世代）：读同目录 default-body.json，经 boot run 提交一条原子 batch。
// 前置：宿主已 start（世界单写者）；本脚本只做确定性构造与提交，可重复执行（同 body 命中 put 幂等，但会追加一条数据世代）。
// 用法：node plugins/router/tools/seed-default-body.mjs --root <宿主根目录>
...
function main() {
      ...
      request: {
        ...
        args: {
          ops: [
            ...
            {
              op: 'add_gen',
              args: { id: 'router', payload: { $n: 0 }, sig: { $n: 0 }, pins: {} },
            },
          ],
        },
        by: 'seed-default',
  ...
  process.stdout.write(`${JSON.stringify({ ok: true, identity: 'router', status: result.status })}\n`)
}
