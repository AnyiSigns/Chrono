...
- **引导页**：整页铺满的居中单列向导（限宽 560px，内容短则垂直居中、长则从头排，滚动条走细窄中性样式）——
  标题区（入口页为眉标 + 欢迎语 + 一句说明，表单页只留入口名）直接接任务区，不设步骤条；不是浮在
  页面上的对话框卡片。
  单色——只取黑 / 白 / 灰 token，不引 accent，层级靠字阶（`--guide-title-size` 大标题）、留白与
  发丝线立；模型区取模型按钮常驻组头右侧（空态即拉取入口，有模型后为刷新入口），模型区两级容器——
  外框 `settings-model-frame` 是装列表的容器（初始高、随条目增至上限后框内滚动），内层
  `settings-model-list` 只承载模型行，空态提示居中于外框内；两级容器建表时各创建一次，之后只重建行，
  不换容器（有模型时外框上方为已选计数 + 全选 / 清空，超过 8 条在批量动作右侧出搜索图标，点开为
  右端与列表容器对齐的窄筛选框，空值失焦或 `Esc` 收回），手填模型 id 收为次级入口；页面主按钮只有
  底部提交一个（`ink` 档，`--c-text` 底 / `--c-bg` 字）；下拉去原生壳换 sprite 箭头，勾选框自绘。
- **文案克制**：引导页只留眉标、标题、一句说明（仅入口页）与操作；入口行卡只留名称与一句说明，
  说明性文字不进表单。
