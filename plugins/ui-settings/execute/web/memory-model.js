...
// 只做数据归一与字符串处理，不触 DOM、不发请求；渲染在 view-memory.js。
...
/** 档位标题文案键。 */
export function layerTitleKey(layer) {
...
export function ttlState(entry) {
...
export function memoryView(result) {
  const empty = { ok: false, at: null, l1: [], l2: [], l3: [] }
  if (!isRecord(result)) return empty
  const list = (key) => (Array.isArray(result[key]) ? result[key].filter((item) => isRecord(item)) : [])
  return {
    ok: result.ok !== false,
 ...
 */
export function filterByWorkspace(entries, layer, workspace) {
...
export function summaryLists(summary) {
  ...
  const list = (key) => (Array.isArray(source[key]) ? source[key].filter((item) => typeof item === 'string' && item.length > 0) : [])
  return {
...
export function memorySearch(result) {
...
export function editSlotPayload(action, layer, id, patch) {
...
export function editTextPatch(text) {
  return { text: typeof text === 'string' ? text : '' }
}

/** 置顶 patch：`pinned` 为 false 表示取消置顶。 */
export function pinPatch(pinned) {
  return { pinned: pinned === true }
}

/** 浏览三态：`degraded`（依赖未就绪）/ `empty`（当前档 + 工作区筛选后无条目）/ `ready`。 */
export function memoryBrowseState(view, degraded, layer, workspace) {
...
export function memorySearchState(busy, degraded, result) {
  if (busy) return 'busy'
  if (degraded) return 'degraded'
