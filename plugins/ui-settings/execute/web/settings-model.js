...
/** 身份只读行（插件页 / 关于页计数）。 */
export function identityRows(ids) {
...
/** 删除一条技能。 */
export function removeSkill(body, id) {
  const base = isRecord(body) ? clone(body) : { version: 1, skills: [] }
...
/** 启停一条技能。 */
export function toggleSkill(body, id, enabled) {
  const base = isRecord(body) ? clone(body) : { version: 1, skills: [] }
  base.skills = (Array.isArray(base.skills) ? base.skills : []).map((item) =>
...
export function skillFromForm(form, id, at) {
