...
export function connectEvents(api: any, ctx: any): () => void {
  if (typeof api?.events?.onAny !== 'function') return () => {}
  return api.events.onAny((record: any) => handleRecord(ctx, record))
}

function handleRecord(ctx: any, record: any): void {
