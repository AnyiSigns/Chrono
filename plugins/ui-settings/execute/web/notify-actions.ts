...
import { normalizePermission, parseNotifyState, permissionFromNotifyGlobal } from './notify.ts'

/** 读同页通知全局（通知前端未加载 → null）。 */
export function readNotifyGlobal(doc: any): any {
  const view = doc.defaultView
  return view === null || view === undefined ? null : (view.__chronoNotify ?? null)
}

/** 拉 `notify.state` 开关；权限取全局发布值（命令未带时）。 */
export async function loadNotify(ctx: any): Promise<any> {
...
export function handleNotifyState(ctx: any, event: any): void {
...
export async function requestPermission(ctx: any): Promise<void> {
