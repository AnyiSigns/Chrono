...
function refEntry(projection, marker) {
...
export function walkTail(projection, listKey, limit = LEDGER_LIMIT) {
 ...
 */
export function healthView(result) {
 ...
 */
export function ledgerLists(source) {
...
export function ledgerSummary(kind, entry) {
  if (!isRecord(entry)) return { id: '', label: '', detail: '' }
  const id = typeof entry.id === 'string' ? entry.id : ''
  const idsOf = (value) => (Array.isArray(value) ? value.filter((item) => typeof item === 'string') : [])
  if (kind === 'verdicts') {
  ...
  const traces = idsOf(entry.traces?.map?.((item) => (isRecord(item) ? item.def : null)))
  const detail = [
    cluster,
...
export function graphView(projection) {
 ...
 */
export function scopeList(projection) {
