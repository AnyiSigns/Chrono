...
import { emptyConfig, isRecord, setUiField, themeCardOf } from './config-model.ts'

/** 当前主题卡片 id（config `ui.theme`，兼容壳落 DOM 的 light / dark 词表）。 */
export function currentThemePref(ctx: any): string {
...
function shellThemePref(card: string): string {
