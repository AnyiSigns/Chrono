...
/** 密钥直写本地文件：`secrets.put`（不进世界、不进审计）。 */
async function handleSecretsPut(
  deps: UiServerDeps,
  req: IncomingMessage,
  res: ServerResponse,
): Promise<void> {
  ...
  const record = isRecord(body) ? body : null
  const name = pickString(record, 'name')
  const value = record === null || typeof record['value'] !== 'string' ? null : (record['value'] as string)
  if (name === null || value === null) {
    sendJson(res, 400, { ok: false, code: 'bad_directive', message: 'name and value required' })
    return
  }
  const result = await deps.bridge.secretsPut(name, value)
  if (!result.ok) {
    sendJson(res, 502, { ok: false, code: result.code, message: result.message })
    return
  }
...
async function handleSecretsDelete(
  ...
  const result = await deps.bridge.secretsDelete(name)
  if (!result.ok) {
    sendJson(res, 502, { ok: false, code: result.code, message: result.message })
    return
  }
  ...
  try {
    switch (route.kind) {
      ...
      case 'api-submit':
        ...
        return
      case 'api-secrets-put':
        await handleSecretsPut(deps, req, res)
        return
