...
const BIN = process.platform === 'win32' ? 'evolve-metrics.exe' : 'evolve-metrics'
...
if (exe === undefined) {
  process.stderr.write(`[evolve-metrics] launch: binary not found; looked in ${candidates.join(', ')}\n`)
  process.exit(127)
}

const child = spawn(exe, process.argv.slice(2), { stdio: 'inherit', windowsHide: true })
child.on('error', (err) => {
  process.stderr.write(`[evolve-metrics] launch: ${err.message}\n`)
  process.exit(127)
})
