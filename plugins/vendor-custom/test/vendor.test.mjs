// vendor-custom 包形状 / 内容测试（零依赖，node --test）。
...
test('custom 无预填（连接值与协议留空 / 缺省）', () => {
  const body = readJson('tools/default-body.json')
  assert.equal(body.default_base_url, '')
  assert.equal(body.default_auth_ref_name, '')
  assert.equal(body.default_reasoning, undefined)
  assert.equal(body.quirks.protocol, null)
  assert.equal(body.quirks.reasoning_field, null)
  assert.equal(body.quirks.reasoning_response_field, null)
  assert.deepEqual(body.quirks.reasoning_map, {})
})
