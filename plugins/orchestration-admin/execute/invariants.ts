...
function fsWrites(contract: Rec): boolean {
  const caps = effectsCaps(contract)
  const fs = isRecord(caps['fs']) ? (caps['fs'] as Rec) : {}
  // 声明式：显式声明 caps.fs.write 为非 'none' 才算写档（与 #33 权威闸口径对齐，2026-09-21 对拍）。
  return typeof fs['write'] === 'string' && fs['write'] !== 'none'
}
...
function riskPorts(view: GraphView, contractIdValue: string, depth: number): string[] {
  ...
  if (contract !== undefined) {
    ...
    // 写档 fs 即高危端口（不要求 `fs` 显式出现在 effects.ports）：与 #33 权威闸口径对齐（2026-09-21 对拍）。
...
function checkFallbackEntry(view: GraphView): GateError[] {
  ...
  for (const contract of view.model.contracts) {
    ...
    const required = contractInputs(contract).filter((input) => input['required'] === true)
    const satisfied = required.every(
      (input) => typeof input['type'] === 'string' && supplyTypes.has(input['type'] as string),
    )
    if (satisfied) return []
  }
  return [
...
function checkApprovalSegment(view: GraphView): GateError[] {
  ...
  for (let i = 0; i < view.n; i++) {
    ...
    if (!ok) {
      errors.push(
        gateError('approval_bypass', `graph.nodes[${i}]`, '高危端口节点的可达路径上缺 guard→approval 段'),
      )
    }
  }
  return errors
...
/** 不变量 6：图中出现的每个契约至少有一个 scope:global 实例；并校验契约存在。 */
function checkGlobalInstance(view: GraphView): GateError[] {
  ...
  for (const id of new Set(graphNodes(view.model.graph))) {
    ...
    const hasGlobal = view.model.nodes.some((node) => {
      if (nodeContractId(node) !== id) return false
      return nodeScope(node)['kind'] === 'global'
    })
    if (!hasGlobal) {
...
/** 四条演化规则（fork-only / diff 上限 / min_runs_before_fork / llm_chain_max 已并入不变量 5）。 */
export function checkEvolution(
  ...
  if (runsSinceFork !== null) {
    ...
    if (runsSinceFork < min) {
      errors.push(
        gateError('min_runs_before_fork', 'graph.derived_from', `攒够 ${min} 回合才能作为下次 fork 的基`),
      )
    }
  }
  return errors
