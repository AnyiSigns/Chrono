...
/// 文件缓存：单文件 `query-vectors.json`，内容为 `{key: value}`；读取失败按空表处理。
...
impl FileStateStore {
    ...
    pub fn from_env() -> Self {
        match std::env::var("CHRONO_PLUGIN_STATE") {
            Ok(dir) if !dir.is_empty() => Self::at(PathBuf::from(dir).join("query-vectors.json")),
    ...
    fn persist(&self, entries: &BTreeMap<String, Value>) {
        ...
        if let Err(err) = atomic_replace(&self.file, text.as_bytes()) {
            crate::frames::log(&format!("query-vectors.json write failed: {err}"));
        }
    }
}
...
fn atomic_replace(path: &std::path::Path, bytes: &[u8]) -> std::io::Result<()> {
    ...
    let name = path
        ...
        .unwrap_or_else(|| "query-vectors.json".to_string());
...
mod tests {
    ...
    fn memory_store_round_trip() {
        ...
        store.write("k", &json!([1.0, 2.0]));
        assert_eq!(store.read("k"), Some(json!([1.0, 2.0])));
    }

    #[test]
    fn file_store_persists_and_recovers() {
        let dir = std::env::temp_dir().join(format!(
            "chrono-memory-retrieval-state-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        let file = dir.join("query-vectors.json");
        {
            let store = FileStateStore::at(file.clone());
            store.write("h1", &json!([0.5, 0.5]));
        }
        let reopened = FileStateStore::at(file);
        assert_eq!(reopened.read("h1"), Some(json!([0.5, 0.5])));
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    fn corrupt_file_is_a_miss() {
        let dir = std::env::temp_dir().join(format!(
            "chrono-memory-retrieval-corrupt-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
    ...
    fn file_store_merges_disk_before_write() {
        ...
        first.write("a", &json!([1.0]));
        // 模拟另一进程（独立实例）：写 b。
        let second = FileStateStore::at(file.clone());
        second.write("b", &json!([2.0]));
        // 第一个实例再写 c：锁内重读盘，不得用旧内存快照覆盖 b。
        first.write("c", &json!([3.0]));
        let reopened = FileStateStore::at(file);
        assert_eq!(reopened.read("a"), Some(json!([1.0])));
        assert_eq!(reopened.read("b"), Some(json!([2.0])));
        assert_eq!(reopened.read("c"), Some(json!([3.0])));
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    fn concurrent_file_writes_do_not_lose_entries() {
        let dir = std::env::temp_dir().join(format!(
            "chrono-memory-retrieval-concurrent-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        ...
        std::thread::scope(|scope| {
            for index in 0..8 {
                let store = &store;
                scope.spawn(move || store.write(&format!("k{index}"), &json!([index as f64])));
            }
        });
        let reopened = FileStateStore::at(file);
        for index in 0..8 {
