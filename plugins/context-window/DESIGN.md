...
- **判据**：L1 条目带 `expires_at`（#3 定，= `at + 24h`）；本插件在候选汇集后、结构化前，按帧 `env.now`（H16）校验 `expires_at <= now` 的 L1 ⇒ **不注入**（记入组装清单 `flags:["l1_expired"]`）；L2 同理按来源记 `l2_expired`（不复用 L1 的 flag）。（2026-09-20 修订）
