...
| 职责 | **结构化文件读写（工作区内 + 工作区外）**，四个工具：`read`（读 / 行窗口）/ `edit`（精确替换，含新建）/ `glob`（按模式找文件）/ `grep`（按模式找内容）；**路径与范围判定（区内 / 区外）是本插件独有职责**（`stat` 的 `mtime` 取自文件系统、不参与确定性保证）（2026-09-20 修订） |
| 依赖 | `->` 25（pins：所有触盘操作经其隔离 / 档位强制）；`<-` 27（pins：按 `tool` 端口契约派发）；**`host` pin**（二进制读写经 `host.asset.put/get`（S1 已落地）；v1 其余路径二进制回 `binary_unsupported`）（2026-09-20 修订）；执行根由 **#14 入口 term 解析写 `bag.workspace_root`（§1.14）**、#27 校验存在性并原样下传（不 pin #41）（2026-09-20 修订） |
...
| 命令 | 无 |
| schema | `schema/tool-fs.json`（工具开关 / 大小与行窗口缺省 / 二进制策略）；**忽略表住本身份数据世代 body**（由调用方 bag 传，§1.14；2026-09-20 修订） |
...
| 验收 | 1) 四个工具经 #27 目录可发现、`argsSchema` 机械可校验；2) **区内**读写按档位放行 / 拒绝；3) **区外**读写按档处理：`auto` 直落、`severe` 升级弹卡、`review` / `deny` 拒；批准后凭一次性 `caps.grant` 只放行本次；4) 形态非法路径（空 / NUL / 非法字符）拒 `bad_path`，档位拒绝 `fs_denied`；5) `read` / `glob` / `grep` 标 `idempotent:false`（文件可变——**不进宿主结果缓存**）、`edit` 永不缓存（2026-09-20 修订）；6) 换 #25 实现不改本插件；7) 结果确定可回放（时间由 bag 传入）；**`stat` 的 `mtime` 取自文件系统、不参与确定性保证**（2026-09-20 修订） |
...
- 根缺失 / 目录已删 → `workspace_missing`（#41 口径）；**`workspace_root` 由 #14 入口 term 解析写 `bag.workspace_root`（§1.14），#27 校验存在性并原样下传；无工作区时绝对路径（区外）仍可派发**（**绝对路径不需要 `workspace_root` 也能解析**）（2026-09-20 修订）。
- 忽略表（`glob` / `grep` 用）住**本身份数据世代 body**（由调用方 bag 传——与 #25 `fsop` 参数对齐，§1.14；**热改 = 数据换代**）；本插件**不提供运行时改它的命令**（2026-09-20 修订）。
...
- **升级判定不在本插件**：本插件只声明"这次是区外 + 读 / 写"；是否弹卡由 #26（本身份数据世代 body `bag.guard_rules` 的启发式，2026-09-20 修订）判。
...
- **二进制 / 大文件 v1 不内联**：命中二进制或超上限 → `binary_unsupported` / `too_large`（结构化失败，不静默）。
  > **宿主能力（S1 已落地）**：`host.asset.put` / `host.asset.get`（服务侧字节存取，规范 base64、8 MiB 内联上限；见 `host.md` §五 宿主扩展面）。**二进制读写经 `host.asset.put/get`（本插件 pin host）；v1 其余路径二进制回 `binary_unsupported`**（`binary_unsupported` 只对分块等后置场景保留）（2026-09-20 修订）。
...
- **宿主能力**：反向帧 `port.call`（已在 `docs/protocol.md` §2.4 落地）；**S1 服务侧资产存取面已落地**（`host.asset.put/get`，**本插件 pin host**，二进制读写前置；v1 其余路径二进制回 `binary_unsupported`，`host.md` §五 宿主扩展面）（2026-09-20 修订）。
