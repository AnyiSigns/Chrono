// `loop-policy` 服务进程入口：宿主服务协议帧循环（docs/protocol.md §二）。
...
const HANDLERS = createHandlers({ port: LINK, host: LINK })
...
function manifest(): Rec {
  return { v: '1', identity: IDENTITY, implements: IMPLEMENTS, methods: METHODS, protocol: PROTOCOL, state: STATE }
}
...
async function handleCall(message: Rec): Promise<void> {
  ...
  try {
    ...
    for (const event of result.events) {
      sendFrame({ v: '1', kind: 'event', topic: event.topic, payload: event.payload })
    }
