// 审批队列与裁决回执的自有持久存储（④ `CHRONO_PLUGIN_DATA`）：队列项 / 游标不再进世界。
// 引擎 = 单文件追加日志 `approval.jsonl`（每条逻辑写一次 append + fsync，换行收尾）；启动重放即得全量队列。
...
/** 队列项 id：`ap-<run>-<seq>`，seq 取入队前的累计计数（确定性、非随机）。 */
export function approvalId(run: string, seq: number): string {
  return `ap-${run}-${seq}`
}

/**
 * 审批队列存储。写口只有本身份：每次逻辑写一条 `write` 记录（边跑边追加），不攒到回合收口。
...
export class ApprovalStore {
  ...
  static open(env: NodeJS.ProcessEnv = process.env): ApprovalStore {
    ...
    if (typeof dataDir === 'string' && dataDir.length > 0) {
      ...
      dataFile = join(dataDir, 'approval.jsonl')
    }
    if (typeof stateDir === 'string' && stateDir.length > 0) {
      mkdirSync(stateDir, { recursive: true })
      stateFile = join(stateDir, 'index.json')
    }
    return new ApprovalStore(dataFile, stateFile)
  }

  /** ③ 派生物：记录水位 + 计数；删了可从 ④ 重放重建（不承载真源）。 */
    ...
    for (const op of ops) {
      ...
      if (op['op'] === 'drop') {
        ...
        for (const raw of ids) {
          ...
          this.items.delete(raw)
          this.order = this.order.filter((id) => id !== raw)
        }
      }
    }
  }
  ...
  // ── 回合标记（半份状态可辨：存在 open 且无 closed 即中断残留） ────────────────
  ...
  /** 按 op_key 找已入队项（幂等收敛用）；无 op_key 或未命中回 null。 */
  ...
  /**
   * 追加一条队列项：`item.id` 由调用方按 `count()` 派生。新项追加到到达序末；同 id 覆盖（幂等）。
   * 累计计数与项在同一逻辑写内落账。
   */
  ...
  /** 从活动队列移除若干项（归档）。 */
  dropItems(run: string | null, ids: string[]): void {
    if (ids.length === 0) return
    this.commit({ t: 'write', run, ops: [{ op: 'drop', ids }] })
  }

  // ── 读口 ────────────────────────────────────────────────────────────────

  /** 活动队列项（到达序 oldest→newest）。 */
  itemsInOrder(): Rec[] {
    const out: Rec[] = []
