...
// 引擎 = 单文件追加日志 `config.jsonl`（每条一次 append + fsync，换行收尾）；启动重放取最后一条 body。
...
/** 配置存储。写口只有本身份：每次变更落一条记录；读从内存态返回（启动重放得到）。 */
export class ConfigStore {
  ...
  static open(env: NodeJS.ProcessEnv = process.env): ConfigStore {
    ...
    if (typeof dataDir === 'string' && dataDir.length > 0) {
      ...
      dataFile = join(dataDir, 'config.jsonl')
    }
    return new ConfigStore(dataFile)
  }
