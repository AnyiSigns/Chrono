...
| 验收 | 1) 读命令返回偏好；2) 各写者直写后立即生效且可回放；3) 主题三档 / 权限四档为枚举；4) 非法写在写入端按 schema 被拒（宿主 v1 不校验身份数据）；5) 单客户端写者经 `config.read` 读-改-写，不同字段不互相覆盖（v1 诚实口径，见「并发语义」）（2026-09-20 修订）；6) 首启默认 body 使 #15 能判「无配置」；7) `providers.<id>.models` 的元数据可被 #12 / #13 / #40 读到；8) 世界与审计中不出现明文密钥 |
...
- **v1 诚实口径**：写者唯一——`#40` / `#17` / `#15` / `#16` 经**同一前端进程**串行提交，写前用 `config.read` 读回、只改自己那一处、整值 `put`（read-modify-write），故不同字段不互相覆盖。
- **框架级字段 CAS / 合并 op 后置**（H12 v1 共享 body 读-改-写、CAS 不触发）：**跨客户端并发写为已知限制**（last-write-wins；config 写罕见，登记接受）。
- 不采用「后写覆盖」也不新增 config 专用写者队列；提交串行化由宿主的单一提交队列提供（`host.md` §五 写者 / H12）。（2026-09-20 修订：删除「CAS 失败重试上限 3、不静默丢字段」这一 v1 不可达承诺）
