// short-memory 包形状 / 内容测试（零依赖，node --test）。
...
test('plugin.json 12 字段齐全且形态合法', () => {
  ...
  assert.equal(decl.schema, 'schema/short-memory.json')
...
test('short-memory schema 是合法 JSON 且符合白名单子集', () => {
  assertWhitelist(readJson('schema/short-memory.json'), 'short-memory.schema')
})

test('short-memory schema 关键字段齐全', () => {
  const schema = readJson('schema/short-memory.json')
  assert.equal(schema.type, 'object')
  assert.deepEqual(schema.required, ['version', 'sessions', 'workspaces'])
  assert.equal(schema.properties.version.type, 'integer')
  assert.equal(schema.properties.sessions.type, 'object')
  assert.equal(schema.properties.sessions.additionalProperties, true)
  assert.equal(schema.properties.workspaces.type, 'object')
  assert.equal(schema.properties.workspaces.additionalProperties, true)
})

test('tools/default-body.json 为空记忆默认体', () => {
