...
// 环境变量 `CHRONO_BROWSER_ENGINE_MODULE` 指向一个导出 `createEngine(config)` 的模块时改用它——
...
async function loadExternalEngine(modulePath: string, config: EngineConfig): Promise<BrowserEngine> {
  ...
  return module.createEngine(config)
}
...
export async function createEngine(
  ...
  if (typeof modulePath === 'string' && modulePath.length > 0) {
    return loadExternalEngine(modulePath, config)
  }
