...
- 截图字节**不进 args / 结果**，走 `{kind:'asset', sha256, mime, size}` 引用。
  > **宿主能力（S1 已落地）**：插件服务存 / 取资产字节（反向可调用的 `host.asset.put` / `host.asset.get`）。本插件尚未实现，验收待插件落地。
...
| `binary_unsupported` | 截图（本插件未实现；S1 资产面已落地） |
...
- **宿主能力**：服务侧资产存取面（截图前置，S1 已落地）；插件 ③ 目录（会话 / 截图暂存，H4 已落地）。
