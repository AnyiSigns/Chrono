...
interface RunWaiter {
  resolve: (frame: Rec | null) => void
  timer: ReturnType<typeof setTimeout>
}

/** 入站面客户端：单连接、自动重连、按 id 配对请求、按 run 等终局结果。 */
export class InboundClient implements Transport {
  ...
  /** 等一次 submit run 的终局 `result` 帧；超时 / 断连回 null（调用方按未确认处理）。 */
  waitForRun(run: string, timeoutMs = this.requestTimeoutMs): Promise<Rec | null> {
    return new Promise<Rec | null>((resolve) => {
      const timer = setTimeout(() => {
        this.runWaiters.delete(run)
        resolve(null)
      }, timeoutMs)
      timer.unref?.()
      this.runWaiters.set(run, { resolve, timer })
    })
  }
  ...
  private onMessage(message: Json): void {
    ...
    if (kind === 'result' && typeof message['run'] === 'string') {
  ...
  private failRunWaiters(frame: Rec | null): void {
