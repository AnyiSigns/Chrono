...
// 本插件 pins 只有 `approval`：命令 / 提交都按名走入站面，eff 由入口 term 发出。
...
export interface Transport {
  ...
  isConnected(): boolean
  /** 等一次 submit run 的终局 `result` 帧（写落账先于后续命令的次序保证）；无能力则缺省。 */
 ...
 * 命令入口 term 若返回计划（裁决 / 列表），业务数据在最后一条 `extern` 条目里，
 * 写条目由宿主自动落账；此处取 extern 载荷作客户端可见值。
 */
...
/** 新请求 id（帧按 id 配对）。 */
export function newRequestId(prefix = 'ui-approval'): string {
