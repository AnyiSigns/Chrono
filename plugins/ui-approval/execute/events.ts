...
// 事件命名空间保持宿主原样（`impl` + `topic`），本页按 topic 过滤（审批条只关心 approval.*）。
...
export function approvalStateRecord(connected: boolean, impl: string): SseRecord {
  return { impl, topic: 'ui-approval.state', payload: { connected } }
}
