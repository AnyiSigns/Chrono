// 反向调用通道（服务 → 宿主，docs/protocol.md §2.4）：本插件 `pins` 含 `approval` → approval，
// 故可按逻辑端口 `approval` 调 `list` / `decide` / `decide_all`（服务不读投影、不发 eff；
// 投影由入口 term 随 args 传入）。
