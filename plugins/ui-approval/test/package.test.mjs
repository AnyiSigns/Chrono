...
test('能力类为 ui-approval（ping 占位 + 三条命令方法）；pins 只一条 approval', () => {
  const decl = readJson('plugin.json')
  assert.deepEqual(decl.implements, ['ui-approval'])
  assert.deepEqual(decl.methods, { 'ui-approval': ['ping', 'list', 'decide', 'decide_all'] })
  assert.deepEqual(decl.pins, { approval: 'approval' })
})

test('members = execute + term；三条命令入口 term 全部存在且无 argsSchema', () => {
  const decl = readJson('plugin.json')
  ...
  assert.deepEqual(
    ...
    ['approval.list', 'approval.decide', 'approval.decide_all'],
  )
