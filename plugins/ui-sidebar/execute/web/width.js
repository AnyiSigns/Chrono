...
export function breakpointOf(viewportWidth) {
...
/** 是否允许展开 / 拉伸（仅宽屏）。 */
export function canResize(viewportWidth) {
  return breakpointOf(viewportWidth) === 'wide'
}

/** 强制收缩：非宽屏一律收缩，忽略用户偏好。 */
export function resolveCollapsed(viewportWidth, userCollapsed) {
  if (!canResize(viewportWidth)) return true
  return userCollapsed === true
}

/** 有效宽度：非宽屏恒为收缩宽度；宽屏按用户偏好取展开（夹取持久值）或收缩。 */
export function effectiveWidth(viewportWidth, storedWidth, collapsed) {
...
export function widthFromDrag(startWidth, deltaX) {
