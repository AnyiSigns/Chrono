...
import { conversationMessages } from './sidebar-model.ts'
...
export function roleLabel(def: any): string {
...
/** 生成 markdown 文本。 */
export function exportMarkdown(conversation: any, messages: any[]): string {
  const title =
    conversation !== null && typeof conversation.title === 'string' && conversation.title.length > 0
      ? conversation.title
      : 'conversation'
  const lines = [`# ${title}`, '']
  if (conversation !== null && typeof conversation.id === 'string') lines.push(`- id: ${conversation.id}`)
  if (conversation !== null && typeof conversation.workspace_id === 'string')
    lines.push(`- workspace: ${conversation.workspace_id}`)
...
/** 生成 JSON 文本。 */
export function exportJson(conversation: any, messages: any[]): string {
  const payload = {
    ...
    workspace_id:
      conversation !== null && typeof conversation.workspace_id === 'string' ? conversation.workspace_id : null,
    exported_at: null,
...
export function messagesOf(history: unknown, conversationId: unknown): any[] {
  return conversationMessages(history, conversationId)
}

/** 文件名安全化：去掉路径分隔与保留字符，空则回落 `conversation`。 */
export function safeFilename(title: unknown): string {
...
/** 导出文件名：`<安全标题>.<ext>`。 */
export function exportFilename(conversation: any, ext: unknown): string {
...
/** 依据格式取正文（`md` / `json`）。 */
export function exportBody(format: unknown, conversation: any, messages: any[]): string {
